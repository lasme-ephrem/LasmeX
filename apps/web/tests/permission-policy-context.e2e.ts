...
import { canonicalPath } from 'lasmex-sandbox'
...
const PROMPTS = [
  ...
  'Does the LasmeX file sandbox currently restrict file operations? Answer directly in one sentence. Do not call tools.',
...
describe('web e2e: current sandbox policy reaches the model before tools', () => {
  ...
  it.skipIf(MODE === 'record')('records cache-safe current policy before the corresponding model behavior', async () => {
    ...
    expect(systems[0]).not.toContain('Current LasmeX file policy:')
    ...
    expect(contexts[0]).toContain('Current LasmeX file policy: read-only. Any available operation enforced by the LasmeX file sandbox cannot modify files in the standing mode.')
    ...
    expect(contexts[1]).toContain('Current LasmeX file policy: danger-full-access. The LasmeX file sandbox does not restrict file modifications by available operations.')
    ...
    expect(contexts[2]).toContain(`Current LasmeX file policy: workspace-write. Any available operation enforced by the LasmeX file sandbox may modify files under the session workspace: ${JSON.stringify(canonicalPath(sessionWorkspace))}. Some platform temporary areas may also be writable.`)
    ...
    expect(contexts[3]).toContain('Current LasmeX file policy: read-only.')
