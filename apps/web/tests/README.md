...
the deliberate composition divergences from `lasmex web` — are documented in
...
## Do not import `lasmex-client-*` here
...
`lasmex-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `lasmex-client-web` and the boot-manifest
type from `lasmex-client-modules/client`: booting the real shell is what
...
`lasmex-client-runtime/client` because `client/runtime` is reachable
