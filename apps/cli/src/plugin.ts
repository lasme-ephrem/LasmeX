/**
 * `lasmex plugin --profile <name> <args...>` — profile plugin management as a
 * thin pnpm forwarder: initialize the profile on first use, run
 * `pnpm <args...>` in the profile directory, then reconcile the
 * `lasmex.profile.bundles` layer list against the installed state (a dependency
 * resolving to a package that declares `lasmex.bundle` joins the layer stack; a
 ...
 * gained its `lasmex.bundle` declaration in a newer version.
 * @module lasmex/plugin
 */
 ...
 * @returns true when the package manifest declares `lasmex.bundle`.
 */
function exportsPatch(packageName: string, profileDir: string): boolean {
  ...
  return manifest.lasmex?.bundle?.patch !== undefined
}

/**
 * Reconcile `lasmex.profile.bundles` against the installed state: pnpm has
 * already written the real installed names (so a git/path/tarball/alias spec
 * on the command line reconciles by its true package name) and materialized
 * the packages. A dependency that resolves to a `lasmex.bundle`-declaring
...
function reconcilePlugins(before: ProfileManifest, profileDir: string): void {
  ...
  const plugins = after.lasmex?.profile?.bundles ?? []
  let changed = false
  for (const packageName of dependencies) {
    ...
    if (isBundle && !plugins.includes(packageName)) {
      ...
      process.stderr.write(
        `${NAME} : avertissement : ${packageName} ne déclare aucun lasmex.bundle — installation comme dépendance simple, pas comme couche du profil `
        + '(une mise à jour qui en ajoute un l’activera automatiquement)\n',
      )
    }
  }
  ...
  for (const packageName of [...plugins]) {
    ...
    // bundles (lasmex-base and friends) are not dependencies.
  ...
  if (!changed) return
  after.lasmex = { ...after.lasmex, profile: { ...after.lasmex?.profile, bundles: plugins } }
 ...
 * @param cwd - the directory `lasmex` was invoked from.
...
/**
 * Run one `lasmex plugin` invocation: init if needed, forward to pnpm, reconcile.
...
export function runPlugin(profile: string, args: readonly string[]): number {
  ...
  if (!existsSync(join(dir, 'package.json'))) {
    ...
    process.stderr.write(`${NAME} : profil ${profile} initialisé dans ${dir}\n`)
  }
  ...
  if (result.error !== undefined) {
    ...
    if (code === 'ENOENT') {
      process.stderr.write(`${NAME} : pnpm est introuvable dans PATH — installez pnpm pour gérer les plugins du profil\n`)
      return 127
    }
    throw result.error
  }
  ...
  if (exitCode === 0) {
    ...
    process.stderr.write(`${NAME} : échec de pnpm dans le dossier de profil ${dir}\n`)
    if (args.some(argument => /^git\+|^github:|\.git(?:#|$)/.test(argument))) {
      process.stderr.write(
        `${NAME} : les plugins hébergés par Git sont construits à l'installation par leur script prepare, que pnpm bloque sans autorisation — `
        + `ajoutez sous allowBuilds, dans ${join(dir, 'pnpm-workspace.yaml')}, la clé exacte affichée ci-dessus par pnpm, puis relancez la commande\n`,
      )
    }
  }
  return exitCode
