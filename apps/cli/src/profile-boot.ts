/**
 * Shared profile boot for every LasmeX surface: resolve the profile, stack its
 * patch layers (bundle layers in `lasmex.profile.bundles` order, the profile's
...
import { LASMEX_LAUNCH_ENVIRONMENT_KEY, type LaunchEnvironmentSnapshot } from 'lasmex-launch-environment'
...
/**
 * The home-level user patch layer (`$LASMEX_HOME/cordis.patch.yml`), applied
 * over every profile's own layer. Resolved per call, not at module load:
 * `$LASMEX_HOME` may be set by the test or launcher after import.
 * @returns the absolute patch-file path.
 */
export function homePatchPath(): string {
  return join(resolveLasmexHome(), PROFILE_PATCH_FILENAME)
}

/** Absolute path of this LasmeX installation's package.json (both anchors: src/ and lib/ sit one level under apps/cli). */
export const INSTALL_ANCHOR = fileURLToPath(new URL('../package.json', import.meta.url))

/** The session-telemetry row id the LASMEX_TELEMETRY_DISABLED switch targets. */
export const TELEMETRY_ROW_ID = 'session-telemetry-otel'

/** The empty root entry list every profile tree patches over. */
const PROFILE_ROOT_CONFIG = `# LasmeX profile root — an empty entry list. The tree is composed as patches:
# each bundle in package.json's lasmex.profile.bundles, then cordis.patch.yml, then any
 ...
 * switch set.
 * @param disabledEnv - the raw `LASMEX_TELEMETRY_DISABLED` value (`undefined` when unset).
...
interface ComposedProfile {
  ...
  /** The home-level user layer (`$LASMEX_HOME/cordis.patch.yml`), applied after the profile's own. */
 ...
 * `lasmex.profile.bundles` order (the base bundle gates the shell stacks by
 * platform on its own rows), the profile's user layer, the home-level user
 * layer (`$LASMEX_HOME/cordis.patch.yml` — machine-local preferences that apply
...
function composeProfile(
  ...
  // The writable root the roster appends is `lasmex-agent-presets`' own, so a
  ...
  const telemetryPatch = resolveTelemetryPatch(process.env.LASMEX_TELEMETRY_DISABLED, rows.has(TELEMETRY_ROW_ID))
...
export interface RunProfileOptions {
  ...
  /** Whether profile and home patch files stay live after boot; defaults to true. */
...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  ...
  const ctx = await boot(NAME, rootConfig, structuredClone(allPatches(composed)), (hostCtx) => {
    ...
    hostCtx.provide(LASMEX_LAUNCH_ENVIRONMENT_KEY, options.environment)
