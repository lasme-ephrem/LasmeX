...
const SESSION_PACKAGE = 'lasmex-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = 'lasmex-session/types'
 ...
 * top-level declaration (in `lasmex-session`) and any declaration
 * merge inside a `declare module 'lasmex-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module lasmex-session/known-event-types',
    ' */',
    '',
    '/**',
