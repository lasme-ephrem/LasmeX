...
function workspaceFixture(options: {
  ...
  writeJson(join(shared, 'package.json'), { name: 'lasmex-shared' })
  writeJson(join(shared, 'tsconfig.json'), {
    extends: '../../../tsconfig.base.json',
    references: [],
  })
  writeJson(join(split, 'package.json'), { name: 'lasmex-split' })
...
describe('Project Reference compiler faces', () => {
  ...
  it('uses the referencing project face throughout the reachable graph', () => {
    ...
    writeJson(join(hostConsumer, 'package.json'), { name: 'lasmex-host-consumer' })
    ...
    writeJson(join(clientConsumer, 'package.json'), { name: 'lasmex-client-consumer' })
