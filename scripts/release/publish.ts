...
/**
 * Build the registry write arguments for one immutable tarball.
 * @param tarball - absolute tarball path.
 * @param version - package version, used to select the prerelease dist-tag.
 * @param provenance - whether npm must create a Sigstore provenance statement.
 * @returns Exact `npm publish` arguments.
 */
export function npmPublishArguments(tarball: string, version: string, provenance: boolean): string[] {
  return [
    'publish',
    tarball,
...
async function publishTarball(
  ...
  for (let tries = 1; tries <= PUBLISH_ATTEMPTS; tries += 1) {
    ...
    const result = attempt('npm', npmPublishArguments(tarball, version, provenance))
    const output = `${result.stdout}${result.stderr}`
...
async function main(): Promise<void> {
  ...
  if (values.family === undefined || values.from === undefined) {
    throw new Error('usage: publish.ts --family <lasmex|vendor> --from <packed directory> [--provenance]')
  }
  ...
  for (const filename of readPublishOrder(directory)) {
    ...
    await publishTarball(tarball, name, version, values.provenance)
