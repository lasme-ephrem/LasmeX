...
function checkManifest(
  ...
  if (owner.packageName === 'lasmex-invariants') return
  if (manifest.peerDependencies?.['lasmex-invariants'] !== 'workspace:^') {
    addViolation(
      violations,
      owner.manifestPath,
      'lasmex-invariants must be a workspace:^ peerDependency',
    )
  }
  if (manifest.devDependencies?.['lasmex-invariants'] !== 'workspace:^') {
    addViolation(
      violations,
      owner.manifestPath,
      'lasmex-invariants must also be a workspace:^ devDependency',
    )
  }
}
...
function checkBuild(
  ...
  if (owner.packageName !== 'lasmex-invariants'
