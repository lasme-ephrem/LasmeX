...
function clientConfigs(id = 'lasmex-client-test') {
...
describe('client bundle build faces', () => {
  it('watches source in development and consumes emitted JavaScript in the Client build', () => {
    const bundle = clientBundle('lasmex-client-test', ['lib/types/index.js'])
...
describe('client bundle purity gate', () => {
  ...
  it('leaves platform table entries and non-scoped specifiers alone', () => {
    expect(resolveId('lasmex-client-ui-slots')).toBeNull()
    expect(resolveId('lasmex-client-web-react')).toBeNull()
    expect(resolveId('lasmex-client-ui-primitives')).toBeNull()
    expect(resolveId('react')).toBeNull()
    expect(resolveId('zod')).toBeNull()
  })

  it('rejects retired table entries (web-react/store left the 8-entry seed)', () => {
    expect(() => resolveId('lasmex-client-web-react/store')).toThrow(/purity/)
  })

  it('lets inline-safe wire layers inline', () => {
    expect(resolveId('lasmex-host-apiproxy/api')).toBeNull()
    expect(resolveId('lasmex-session/surface')).toBeNull()
    expect(resolveId('lasmex-brand')).toBeNull()
  })

  it('lets exact generated Remote contributions inline without admitting their package implementation', () => {
    expect(resolveId('lasmex-goal/remote')).toBeNull()
    expect(() => resolveId('lasmex-goal')).toThrow(/purity/)
    expect(() => resolveId('lasmex-goal/client')).toThrow(/purity/)
    expect(() => resolveId('lasmex-goal/remote/nested')).toThrow(/purity/)
  })

  it('throws on any other LasmeX workspace leak', () => {
    expect(() => resolveId('lasmex-agent')).toThrow(/purity/)
    expect(() => resolveId('lasmex-client-web')).toThrow(/purity/)
  })

  it('throws on cross-plugin value imports — bare plugin names and /client subpaths alike (the rewrite arm is gone)', () => {
    expect(() => resolveId('lasmex-client-connection')).toThrow(/purity/)
    expect(() => resolveId('lasmex-client-runtime')).toThrow(/purity/)
    expect(() => resolveId('lasmex-client-ui-layout/client')).toThrow(/purity/)
  })

  it('carries exactly one documented temporary exemption: runtime/client (store engine pending rehoming)', () => {
    expect(resolveId('lasmex-client-runtime/client')).toBeNull()
    const clientChannels = CLIENT_EXTERNALS.filter(
      entry => entry.startsWith('lasmex-') && entry.endsWith('/client'))
    expect(clientChannels).toEqual(['lasmex-client-runtime/client'])
  })
})
...
describe('client bundle debug artifacts', () => {
  ...
  it('maps first-party sources to their repository package paths', () => {
    const configs = clientConfigs('lasmex-client-ui-goal')
    ...
    const resolved = new URL(source, 'https://dsh.test/plugins/lasmex-client-ui-goal/client.js.map')
  ...
  it('maps dual-face host sources to the host package group', () => {
    const configs = clientConfigs('lasmex-host-directory-picker-native')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
  ...
  it('maps inlined workspace sources to packages and leaves dependencies outside it unchanged', () => {
    const configs = clientConfigs('lasmex-client-connection')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
    ...
    const resolved = new URL(workspaceSource, 'https://dsh.test/plugins/lasmex-client-connection/client.js.map')
