...
def _host_platform_tag() -> str:
    ...
    try:
        ...
        raise RuntimeError(f"unsupported lasmex-runtime-bin build platform: {key}") from exc
...
class RuntimeBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict[str, object]) -> None:
        ...
        if self.target_name == "sdist":
            raise RuntimeError(
                "lasmex-runtime-bin is wheel-only; build and publish platform wheels only."
            )
        ...
        runtime_dir = Path(self.root) / "src" / "lasmex_runtime" / "runtime"
        runtime_files = sorted(runtime_dir.glob("lasmex-jsonrpc-agent-pkg-*") if runtime_dir.is_dir() else [])
