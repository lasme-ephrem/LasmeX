...
class LasmeXClientConfig:
    """Configuration for launching the local LasmeX SDK runtime."""
...
class LasmeXClient:
    """Synchronous JSON-RPC client for the LasmeX SDK runtime over stdio."""

    def __init__(self, config: LasmeXClientConfig | None = None) -> None:
        self.config = config or LasmeXClientConfig()
    ...
    def __enter__(self) -> "LasmeXClient":
        self.start()
        return self
    ...
    def close(self) -> None:
        ...
        try:
            ...
            proc.wait()
        self._proc = None
        self._fail_waiters(self._runtime_closed_error("LasmeX runtime closed"))
            ...
            while True:
                ...
                if deadline is not None:
                    ...
                    if remaining <= 0:
                        ...
                        raise TimeoutError(
                            f"{method} timed out waiting for LasmeX runtime{suffix}"
                        )
    ...
    def _write_message(self, message: JsonObject) -> None:
        ...
        if proc is None or proc.stdin is None:
            raise TransportClosedError("LasmeX runtime is not running")
        try:
            ...
            raise self._runtime_closed_error("Failed to write to LasmeX runtime") from exc

    def _start_reader_thread(self) -> None:
        self._reader_thread = threading.Thread(target=self._reader_loop, name="lasmex-runtime-reader", daemon=True)
        self._reader_thread.start()

    def _start_stderr_thread(self) -> None:
        self._stderr_thread = threading.Thread(target=self._stderr_loop, name="lasmex-runtime-stderr", daemon=True)
    ...
    def _reader_loop(self) -> None:
        ...
        try:
            ...
            self._fail_waiters(self._runtime_closed_error("LasmeX runtime stdout closed"))
    ...
    def _default_launch_args(self) -> tuple[str, ...]:
        ...
        try:
            from lasmex_runtime import resolve_bundled_launch_args
        except ImportError as exc:
            raise FileNotFoundError(
                "Unable to locate the bundled LasmeX SDK runtime. "
                "Install lasmex-runtime-bin or set LasmeXClientConfig.runtime_bin."
            ) from exc
    ...
    def _inject_bundled_default_config(self, env: dict[str, str]) -> None:
        ...
        if not uses_bundled_runtime or env.get("LASMEX_CORDIS_CONFIG"):
            return
        # _default_launch_args already imported the package or raised its install error.
        from lasmex_runtime import bundled_default_config_path

        env["LASMEX_CORDIS_CONFIG"] = str(bundled_default_config_path())
