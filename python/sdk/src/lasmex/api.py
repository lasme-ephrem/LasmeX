...
class LasmeX:
    """Reusable synchronous SDK for running LasmeX agent turns.
    ...
    def __init__(self, config: LasmeXConfig | None = None, **kwargs: object) -> None:
        if config is not None and kwargs:
            raise TypeError("pass either LasmeXConfig or keyword options, not both")
        self.config = config or LasmeXConfig(**kwargs)
        ...
        if self.config.session_root is not None:
            env["LASMEX_SESSION_ROOT"] = self.config.session_root
        if self.config.cordis is not None:
            env["LASMEX_CORDIS_CONFIG"] = self.config.cordis
        env["LASMEX_CWD"] = cwd
...
class Session:
    def __init__(self, lasmex: LasmeX, session_id: str) -> None:
        self.lasmex = lasmex
        self.id = session_id
    ...
    def run(
        ...
        *,
        ...
        with self.lasmex.client.subscribe_session_notifications(self.id) as subscription:
            message_id = self.lasmex.client.session_prompt(
                self.id,
                content_blocks,
        ...
        return RunResult(
            ...
            session_root=self.lasmex.config.session_root,
        )
