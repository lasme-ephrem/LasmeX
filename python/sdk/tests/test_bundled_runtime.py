...
from lasmex import LasmeX, LasmeXClient, LasmeXClientConfig
from lasmex.errors import TransportClosedError
...
- id: sdk-jsonrpc-server
  name: 'lasmex-sdk-jsonrpc-server'
- id: agent-core
  name: 'lasmex-agent-spine-demo'
  config:
...
- id: session-checkpoints
  name: 'lasmex-session-checkpoint-policy'
- id: subprocess
  name: 'lasmex-subprocess-local'
- id: bash
...
def _client(tmp_path: Path, launch_args: tuple[str, ...]) -> LasmeXClient:
    return LasmeXClient(
        LasmeXClientConfig(
            launch_args_override=launch_args,
            cwd=str(tmp_path),
            env={
                "LASMEX_CORDIS_CONFIG": "./cordis.yml",
                "LASMEX_SESSION_ROOT": str(tmp_path / "sessions"),
                "LASMEX_CWD": str(tmp_path),
...
def test_bundled_runtime_boots_a_cordis_config(tmp_path: Path, mode: str) -> None:
    ...
    assert init.serverInfo.name == "lasmex-sdk-runtime"
...
def test_python_sdk_boots_minimal_jsonrpc_config(tmp_path: Path, mode: str) -> None:
    ...
    harness = LasmeX(
        ...
        env={
            "LASMEX_MODEL": model,
            "LASMEX_CONTEXT_WINDOW": "1000000",
            "LASMEX_SYSTEM_PROMPT": "You are the Python SDK minimal boot test agent.",
        },
...
def test_bundled_runtime_surfaces_unbundled_plugin_failure(tmp_path: Path, mode: str) -> None:
    ...
    assert "lasmex-does-not-exist" in str(excinfo.value)
...
def test_zero_config_run_injects_bundled_default_cordis_config(
    ...
    monkeypatch.setenv("LASMEX_RUNTIME_MODE", mode)
