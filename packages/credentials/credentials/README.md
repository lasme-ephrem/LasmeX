...
[`lasmex-credentials-local`](../credentials-local/README.md) layers the inherited process environment over its managed `$LASMEX_HOME/.credentials.yaml` document, with the launcher's project and user `.env` layers as fallbacks. The seam shape leaves room for keyring-, helper-command-, and KMS-backed providers; a remote settings provider never needs to carry secrets.
