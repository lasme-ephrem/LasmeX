...
import { canonicalPath, type SandboxExecutionPolicy, type SandboxMode } from 'lasmex-sandbox'
...
function renderPolicyContext(policy: SandboxExecutionPolicy): string {
  switch (policy.mode) {
    case 'read-only':
      return 'Current LasmeX file policy: read-only. Any available operation enforced by the LasmeX file sandbox cannot modify files in the standing mode. Do not refuse a required modification from this policy alone: try an available tool normally and follow any denial and escalation guidance it returns.'
    case 'workspace-write':
      return `Current LasmeX file policy: workspace-write. Any available operation enforced by the LasmeX file sandbox may modify files under the session workspace: ${JSON.stringify(policy.workspaceRoot)}. Some platform temporary areas may also be writable.`
    case 'danger-full-access':
      return 'Current LasmeX file policy: danger-full-access. The LasmeX file sandbox does not restrict file modifications by available operations.'
