...
import SandboxPolicyService, { SANDBOX_MODES, effectiveSandboxMode, setSandboxMode } from 'lasmex-sandbox-policy'
import SystemPrompt, { renderContextSnapshot, renderPrompt } from 'lasmex-system-prompt'
...
describe('sandbox:policy request context', () => {
  ...
  it.each(['read-only', 'workspace-write', 'danger-full-access'] as const)('renders the exact %s policy without a capability inventory', async (mode) => {
    ...
    const expected = {
      'read-only': 'Current LasmeX file policy: read-only. Any available operation enforced by the LasmeX file sandbox cannot modify files in the standing mode. Do not refuse a required modification from this policy alone: try an available tool normally and follow any denial and escalation guidance it returns.',
      'workspace-write': `Current LasmeX file policy: workspace-write. Any available operation enforced by the LasmeX file sandbox may modify files under the session workspace: ${JSON.stringify(workspaceRoot)}. Some platform temporary areas may also be writable.`,
      'danger-full-access': 'Current LasmeX file policy: danger-full-access. The LasmeX file sandbox does not restrict file modifications by available operations.',
    } as const
  ...
  it('reflects the latest durable switch on the next assembly and stays byte-stable otherwise', async () => {
    ...
    expect(danger).toBe('Current LasmeX file policy: danger-full-access. The LasmeX file sandbox does not restrict file modifications by available operations.')
    expect(await policyContext(ctx, active)).toBe(danger)

    setSandboxMode(active, 'workspace-write')
    expect(await policyContext(ctx, active)).toBe(`Current LasmeX file policy: workspace-write. Any available operation enforced by the LasmeX file sandbox may modify files under the session workspace: ${JSON.stringify(resolve('/projects/current'))}. Some platform temporary areas may also be writable.`)
  })
