...
import { contentHasImage, createUserMessage, freezeMessage, ReasoningEffortId } from 'lasmex-llm'
import { errorChain } from 'lasmex-llm'
import type { ContentBlock, MessageSource } from 'lasmex-llm'
import { isAppendSurfaceEvent, isJsonValue } from 'lasmex-session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from 'lasmex-session'
import type { SessionPersistence } from 'lasmex-session-persistence'
import { SessionQueryError, type SessionSearchCursor } from 'lasmex-session-query'
import { SubagentError } from 'lasmex-subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from 'lasmex-subagent'
import { isUserInvocable } from 'lasmex-skill'
...
} from 'lasmex-agent-presets'
import type { PresetBearingSession } from 'lasmex-agent-presets'
...
import type { SessionRawArtifact } from 'lasmex-session-persistence'
import {
...
import type {} from 'lasmex-jobs'
import type { JobSnapshot } from 'lasmex-jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from 'lasmex-session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from 'lasmex-goal'
import type { GoalRef as CoreGoalRef } from 'lasmex-goal'
...
import type {} from 'lasmex-cordis-host-runner/types'
import type {} from 'lasmex-skill'
// The settings/credentials seams: brand guards run at this wire boundary; the
// service reads stay optional (`ctx.get`) so a composition without either
// provider still serves every other domain.
import { SettingsConflictError, settingsNamespace } from 'lasmex-settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from 'lasmex-settings'
import { credentialRef } from 'lasmex-credentials'
// Value edge: the rename impl narrows the title service's validation failure; the import also resolves `ctx.get('sessionTitle')`.
import { SessionTitleInvalidError } from 'lasmex-session-title'
import type { CallId } from 'lasmex-llm/brand'
import type { ScopeKey } from 'lasmex-scope'
import type { ApprovalOutcome, ApprovalRequestId } from 'lasmex-user-approval'
...
} from 'lasmex-user-questions'
import { UserQuestionError } from 'lasmex-user-questions'
import { DirectoryPickerError } from 'lasmex-host-directory-picker'
import {
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load lasmex-session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  // attachment Service Definition: lasmex-llm depends on lasmex-attachment, so the
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts lasmex-goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. lasmex-settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. lasmex-credentials-local) in its composition', details: {} }
  }
  ...
  return {
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount lasmex-session-query',
            details: {},
          })
        }
        try {
    ...
    agentPresets: {
      ...
      // Authoring is privileged (see PRIVILEGED_METHODS in lasmex-client-connection):
    ...
    skills: {
      ...
      async list(request) {
        ...
        // composition mounts lasmex-skill, not an empty catalog. `ctx.get` also
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts lasmex-skill', details: {} })
        }
