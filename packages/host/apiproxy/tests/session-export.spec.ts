...
import type { SessionLineageNode } from 'lasmex-session-query'
import type { SessionRawArtifact } from 'lasmex-session-persistence'
import ApiProxyService, { createApiProxy, toFetchHandler } from 'lasmex-host-apiproxy'
...
describe('session.export download endpoint', () => {
  it('streams a ZIP with the root artifact verbatim under its original filename', async () => {
    ...
    expect(response.headers.get('content-disposition')).toContain('lasmex-session-session-root.zip')
