...
import { FileSystem, FsError, FsVersion, type FsDirEntry, type FsEditOutcome, type FsEditRequest, type FsInfo, type FsPathInfo, type FsTarget, type FsWriteOutcome } from 'lasmex-fs'
...
describe('lasmex-skill-filesystem plugin exports', () => {
...
describe('FileSystemSkillProvider', () => {
  it('discovers project, custom, user, and agents skill roots in priority order', async () => {
    ...
    await writeSkill(join(home, '.lasmex/skills'), 'same', 'user dsh skill')
    await writeSkill(custom, 'same', 'custom skill')
    await writeSkill(join(project, '.agents/skills'), 'same', 'project agents skill')
    await writeSkill(join(project, '.lasmex/skills'), 'same', 'project dsh skill')
    await writeSkill(custom, 'custom-only', 'custom only')
    await writeSkill(join(home, '.lasmex/skills/.system'), 'hidden-system', 'hidden system')
    ...
    expect(skills.find(skill => skill.name === 'same')?.source).toBe('project-lasmex')
    ...
    await writeSkill(join(noGit, '.lasmex/skills'), 'fallback-root', 'Fallback root')
  ...
  it('lets project skills override runtime while runtime overrides custom and user skills', async () => {
    ...
    await writeSkill(join(project, '.lasmex/skills'), 'project-name', 'Project wins')
    await writeSkill(custom, 'runtime-name', 'Custom loses')
    await writeSkill(join(home, '.lasmex/skills'), 'runtime-name', 'User loses')
  ...
  it('parses flat skills and filters invalid skills from the invocation-neutral listing', async () => {
    ...
    const root = join(home, '.lasmex/skills')
  ...
  it('discovers symlinked skill directories and flat files', async () => {
    ...
    await mkdir(join(home, '.lasmex/skills'), { recursive: true })
    await symlink(join(external, 'linked-dir'), join(home, '.lasmex/skills/linked-dir'))
    await symlink(join(external, 'linked-flat.md'), join(home, '.lasmex/skills/linked-flat.md'))
    await symlink(join(external, 'missing'), join(home, '.lasmex/skills/broken-link'))
    await symlink('/dev/null', join(home, '.lasmex/skills/device-link'))
  ...
  it('uses the filesystem service for discovery, reads, and project-root lookup', async () => {
    ...
    await ctx.plugin(SkillRegistry)
    await ctx.plugin(SkillFileSystem, { lasmexHome: join(home, '.lasmex'), agentsHome: join(home, '.agents'), watch: false })

    expect((await ctx.skills.list({ cwd: nestedCwd })).map(skill => [skill.name, skill.source])).toEqual([
      ['backend-root', 'project-agents'],
      ['text-skill', 'user-lasmex'],
    ])
  ...
  it('forwards cancellation to filesystem reads while loading a skill', async () => {
    ...
    await writeSkill(join(home, '.lasmex/skills'), 'abortable-skill', 'Abortable skill')
  ...
  it('uses fs/observed as a synchronous first-party invalidation path without a watcher', async () => {
    ...
    emitObserved(join(home, '.lasmex/skills/.system/SKILL.md'), { name: 'write' })
  ...
  it('uses default home root resolution without exposing builtin skills', async () => {
    const previousLasmexHome = process.env.LASMEX_HOME
    const previousAgentsHome = process.env.LASMEX_AGENTS_HOME
    const previousBundledSkillDir = process.env.LASMEX_BUNDLED_SKILL_DIR
    const envHome = await tempDir('skill-env-home')
    try {
      process.env.LASMEX_HOME = join(envHome, '.lasmex')
      process.env.LASMEX_AGENTS_HOME = join(envHome, '.agents')
      const bundled = join(envHome, 'bundled-skills')
      process.env.LASMEX_BUNDLED_SKILL_DIR = bundled
      await writeSkill(join(envHome, '.lasmex/skills'), 'env-skill', 'Env skill')
      ...
      process.env.LASMEX_HOME = join(envHome, 'empty-dsh')
      delete process.env.LASMEX_BUNDLED_SKILL_DIR
      process.env.LASMEX_AGENTS_HOME = join(envHome, 'empty-agents')
      ...
      expect(new SkillFileSystem.FileSystemSkillProvider(empty, {
        ...
        invalidate() {},
      }, { lasmexHome: join(envHome, 'empty-dsh') }).name).toBe('filesystem')
    } finally {
      if (previousLasmexHome === undefined) {
        delete process.env.LASMEX_HOME
      } else {
        process.env.LASMEX_HOME = previousLasmexHome
      }
      if (previousAgentsHome === undefined) {
        delete process.env.LASMEX_AGENTS_HOME
      } else {
        process.env.LASMEX_AGENTS_HOME = previousAgentsHome
      }
      if (previousBundledSkillDir === undefined) {
        delete process.env.LASMEX_BUNDLED_SKILL_DIR
      } else {
        process.env.LASMEX_BUNDLED_SKILL_DIR = previousBundledSkillDir
      }
    }
  })
})
