 ...
 *
 * @module lasmex-skill-filesystem
 */
...
import type { FileSystem, FsDirEntry, FsTarget } from 'lasmex-fs'
...
const PROJECT_LASMEX_RANK = 100
const PROJECT_AGENTS_RANK = 200
const CUSTOM_RANK = 300
const USER_LASMEX_RANK = 400
...
export class FileSystemSkillProvider implements SkillProvider {
  ...
  constructor(
    ...
    this.agentsHome = resolve(config.agentsHome ?? process.env.LASMEX_AGENTS_HOME ?? join(homedir(), '.agents'))
    ...
    const bundledSkillDir = config.bundledSkillDir
      ?? (this.includeDefaultRoots ? process.env.LASMEX_BUNDLED_SKILL_DIR : undefined)
  ...
  private async roots(cwd: string | undefined): Promise<SkillRoot[]> {
    ...
    if (this.includeDefaultRoots && cwd !== undefined) {
      ...
      roots.push(
        { path: join(projectRoot, '.lasmex/skills'), source: 'project-lasmex', rank: PROJECT_LASMEX_RANK, projectRoot },
    ...
    if (this.includeDefaultRoots) {
      roots.push(
        { path: join(this.lasmexHome, 'skills'), source: 'user-lasmex', rank: USER_LASMEX_RANK, skipSystem: true },
