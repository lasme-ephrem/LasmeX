...
import type { ImageAttachmentLimits } from 'lasmex-attachment'
...
async function root(): Promise<string> {
  const value = await mkdtemp(join(tmpdir(), 'lasmex-attachment-'))
  roots.push(value)
...
describe('local attachment store', () => {
  it.skipIf(process.platform === 'win32')('syncs every object ancestor up to the durable boundary before returning', async () => {
    ...
    // Each process first proves LASMEX_HOME durable all the way to the filesystem
