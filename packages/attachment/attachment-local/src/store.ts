...
/**
 * Establish this process's proof that one LASMEX_HOME entry and every ancestor
 ...
 * @param root - absolute `LASMEX_HOME/attachments/v1` root.
 * @param input - encoded bytes and declared metadata.
 * @param limits - resolved storage policy.
...
export async function saveImageFile(root: string, input: SaveImageAttachment, limits: ImageAttachmentLimits): Promise<ImageAttachmentRef> {
  ...
  // Establish LASMEX_HOME itself against the filesystem root once per process.
