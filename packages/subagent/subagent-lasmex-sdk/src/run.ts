/**
 * Fresh-process SDK subagent client. Drives one child LasmeX
 * runtime over stdio JSON-RPC through `lasmex-sdk-client` and owns
 * cancellation and quiescent disposal. Structure mirrors the ACP backend
 * (`lasmex-subagent-acp`): publish after the child handshake,
 ...
 *
 * @module lasmex-subagent-lasmex-sdk/run
 */

import { randomUUID } from 'node:crypto'
import { type HarnessNotification, LasmeX } from 'lasmex-sdk-client'
import type { ContentBlock } from 'lasmex-llm'
import { SessionId, type SessionEvent, type TurnEndReason } from 'lasmex-session'
import type { SubagentResult, SubagentRun, SubagentStartRequest, SubagentStopReason } from 'lasmex-subagent'
import { AssistantOutputFold, settleRunResult, subprocessRunHandle } from 'lasmex-subagent'
import { scrubbedParentEnv } from 'lasmex-subprocess'

/** Resolved spawn spec for an SDK runtime child process (no defaults — see Config). */
export interface SdkRunSpec {
  /** The executable to spawn (the child runtime — a `lasmex-jsonrpc-agent` bin or packaged exe). */
  command: string
   ...
   * runtime's own `DEEPSEEK_API_KEY`, or `LASMEX_CORDIS_CONFIG`). Merged after
   * the seam's `scrubbedParentEnv()` base, so an explicit credential or
   * current `LASMEX_*` fact survives while ambient namesakes never leak.
   */
