 ...
 * @module lasmex-subagent-acp/run
 */
...
import { AssistantOutputFold } from 'lasmex-subagent'
...
export interface AcpRunSpec {
   ...
   * Explicit `LASMEX_*` entries are deployment-owned facts for the child harness
   * (e.g. `LASMEX_PERMISSION_MODE`); they simply merge after the scrub that
...
export async function startAcpRun(request: SubagentStartRequest, spec: AcpRunSpec): Promise<SubagentRun> {
  ...
  // to the result. The seam's scrub drops ambient credentials and LASMEX_* names
