...
async function setupWith(adapter: MockAdapter | GatedAdapter) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'lasmex-tool-subagent-control-'))
  roots.push(root)
...
describe('lasmex-tool-subagent-control', () => {
...
describe('lasmex-tool-subagent-control interrupt_agent', () => {
