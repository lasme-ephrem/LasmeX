 ...
 *
 * @module lasmex-session-query-sqlite/tests/load-path
 */

import { afterEach, describe, expect, it } from 'vitest'
import { Context } from '@deepseek-ai/cordis'
import Loader from '@deepseek-ai/cordis-plugin-loader'
import { SESSION_FORMAT_VERSION, SessionId } from 'lasmex-session'
import SessionStore from 'lasmex-session'
import SqliteSessionPersistence from 'lasmex-session-persistence-sqlite'
import SqliteSessionQueryEngine, * as queryModule from 'lasmex-session-query-sqlite'
...
async function temporaryPath(name: string): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'lasmex-session-search-loader-'))
  temporaryDirectories.push(directory)
  return join(directory, name)
}

describe('lasmex-session-query-sqlite real Loader path', () => {
