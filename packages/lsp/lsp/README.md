...
| Package | Role |
|---|---|
| `lasmex-lsp` (this) | Service Definition: the service, provider registry keyed by branded id + extension mapping, per-query selection, request/result vocabulary, the `LspError` taxonomy |
| `lasmex-lsp-stdio` | Service Provider: a generic local backend that registers configured stdio language-server providers |
| `lasmex-tool-lsp` | Consumer: the model-facing `lsp` tool over `ctx.lsp` |
...
Providers register **capabilities**, not tools. `lasmex-tool-lsp` is the only owner of the model-facing name, description, prompt guidance, schema, and presentation.
...
Indirectly, through `lasmex-tool-lsp`, which owns the model-facing `lsp` schema, prompt, and rendered results while this registry contributes no prompt or schema itself.

#### KV Cache effect

No direct invalidation; `lasmex-tool-lsp` owns request-prefix changes.
