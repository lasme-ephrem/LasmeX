...
import { LspConnection } from 'lasmex-lsp-stdio'
import type { ConnectionWriter } from 'lasmex-lsp-stdio/src/connection.ts'
import { scrubbedParentEnv } from 'lasmex-subprocess'
import { spawnSubprocess } from 'lasmex-subprocess-local/src/spawn.ts'
...
describe('LspConnection', () => {
  ...
  it('forwards explicit LASMEX_* env entries to the child', async () => {
    // A configured LASMEX_* fact must reach the child: the seam scrubs only the
    // ambient namespace, and the explicit entry merges after that scrub. The
    // fixture echoes the named variable back as hover text.
    const conn = connect({ LSP_FAKE_ECHO_ENV: 'LASMEX_LSP_TEST_FACT', LASMEX_LSP_TEST_FACT: 'managed' })
