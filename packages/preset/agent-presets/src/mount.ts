 ...
 * @module lasmex-agent-presets/mount
 */
...
import { scopeOf, scopeParentOf, type ScopeKey } from 'lasmex-scope'
...
class PresetTree extends Include {
   ...
   * dependencies, so every `lasmex-*` row would fail to import. The
 ...
 * invariant companion's service listener, and `lasmex-invariants` is a
