 ...
 * @module lasmex-agent-presets
 */

import { stat } from 'node:fs/promises'
import { Context, Service } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import { bindScopeParent, createScope, scopeOf, type Scope, type ScopeKey, type ScopeParentBinding } from 'lasmex-scope'
// Type-only: resolves the `agent/created` lifecycle event this service watches.
import type {} from 'lasmex-agent'
import { settingsNamespace, type SettingsScope, type default as SettingsService } from 'lasmex-settings'
...
export class AgentPresets extends Service {
  ...
  constructor(ctx: Context, public config: Config) {
    ...
    this.resolvedRoots = config.includeUserRoot
      ? [...config.roots, { path: lasmexHomePath(USER_PRESET_DIR), trust: 'user' }]
      : [...config.roots]
   ...
   * scope key. The binding is lasmex-scope's only re-link capability; holding it
  ...
  private async ensureStanding(preset: AgentPreset): Promise<StandingMount> {
    ...
    if (pending !== undefined) {
      ...
      // it is gone. The subtree is not inert — `lasmex-skill-filesystem` watches its
