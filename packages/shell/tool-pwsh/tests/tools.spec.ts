 ...
 * managed `LASMEX_*` collection, abort translation, canonical result projection,
...
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellRunResult } from 'lasmex-shell'
...
import type { ShellProcessRead } from 'lasmex-shell'
...
async function setup(toolConfig: Partial<ToolPwsh.Config> = {}, lasmexHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
  await ctx.plugin(BashEnvPlugin, lasmexHome === undefined ? {} : { lasmexHome })
...
async function setupWithTasks(toolConfig: Partial<ToolPwsh.Config> = {}, lasmexHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
...
describe('execution through the bash seam', () => {
  it('forwards command, session cwd, timeout, and managed LASMEX_* environment', async () => {
    const lasmexHome = mkdtempSync(join(tmpdir(), 'lasmex-tool-pwsh-home-'))
    const { ctx, bash } = await setup({}, lasmexHome)
    ...
    expect(request?.lasmexEnv).toEqual({
      LASMEX_HOME: lasmexHome,
      LASMEX_SHELL: '1',
      LASMEX_SESSION_ID: 'session-1',
    })
  ...
  it('omits workdir and the session id without an agent, so executor defaulting applies', async () => {
    ...
    const lasmexEnv = bash.requests[0]?.lasmexEnv
    expect(lasmexEnv).toBeDefined()
    expect(lasmexEnv?.['LASMEX_SHELL']).toBe('1')
    expect(lasmexEnv?.['LASMEX_HOME']).toEqual(expect.any(String))
    expect(lasmexEnv).not.toHaveProperty('LASMEX_SESSION_ID')
  })
...
describe('per-call sandbox policy resolution', () => {
  it('stamps the CALLING SESSION\'s resolved policy onto the request (session cwd, not the server launch dir)', async () => {
    ...
    const sessionCwd = mkdtempSync(join(tmpdir(), 'lasmex-tool-pwsh-policy-'))
