 ...
 *
 * @module lasmex-pwsh-local
 */

/* jscpd:ignore-start -- this executor mirrors lasmex-bash-local call-for-call by
   design (see this package's README), so the two import the same seam surface */
import { Context } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import { SHELL_SETTINGS_NAMESPACE, ShellExecutor } from 'lasmex-shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult, CollectedOutput } from 'lasmex-shell'
import type { SubprocessCollect, SubprocessHandle, SubprocessOutputReader, SubprocessSpawnSpec } from 'lasmex-subprocess'
import { installSettingsSection } from 'lasmex-settings'
import { clampTimeout, deadline, MAX_TIMER_DELAY_MS, timeoutOf } from 'lasmex-timeout'
/* jscpd:ignore-end */
import { resolvePwshPath } from './resolve.ts'

/* jscpd:ignore-start -- deliberate call-for-call mirror of lasmex-bash-local (Agent Note: pwsh-tool-and-executor). */
/**
...
export class PwshLocalExecutor extends ShellExecutor {
   ...
   * `lasmex-bash-local`'s `runArgv`/`startArgv` hooks; see
   * `lasmex-pwsh-sandbox`).
   */
  ...
  private spawnSpec(
    ...
    return {
      ...
      signal,
      env: { ...ENV_OVERRIDES, ...spec.env, ...spec.lasmexEnv },
    }
  }
   ...
   * `lasmex-bash-local` (whose sandboxing subclass consumes the same hook); the
   * pwsh-confining consumer is `lasmex-pwsh-sandbox`.
