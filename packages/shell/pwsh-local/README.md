...
The Windows counterpart of `lasmex-bash-local`, deliberately mirroring its semantics call-for-call:
...
- **Model-friendly terminal env** — `NO_COLOR=1 PAGER=cat GIT_PAGER=cat` (no `TERM=dumb`: that is a POSIX concept; `NO_COLOR` is honored by modern PowerShell renderers) merged as ordinary env under the service's credential scrub and `LASMEX_*` channel rules; an explicit caller entry still wins.
...
Indirectly, through `lasmex-tool-pwsh`, which renders this executor's bounded stdout/stderr tails, background-process deltas (through the generic job runtime), spill-file paths, and infrastructure failures.
