...
import { ShellExecutor } from 'lasmex-shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult } from 'lasmex-shell'
import SystemPrompt from 'lasmex-system-prompt'
import ToolRuntime, { TOOL_ABORTED, TOOL_ABORTED_BEFORE_DISPATCH } from 'lasmex-tools'
...
import type { ApprovalOutcome } from 'lasmex-user-approval'
...
const spillDir = mkdtempSync(join(tmpdir(), 'lasmex-tool-bash-spec-'))
...
describe('background execution through the job runtime', () => {
  ...
  it('fails loud when the job runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load lasmex-jobs and lasmex-tool-jobs')
  })
...
describe('the model-facing bash tool builds its request from named args only (no {...args} forward)', () => {
  const recordingLasmexHome = join(spillDir, 'dsh-home')
   ...
   * (the credential scrub in lasmex-bash-local is the security control; see the
  ...
  class RecordingBashExecutor extends ShellExecutor {
    ...
    resolve(request: ShellExecRequest): ShellExecSpec {
      ...
      return {
        ...
        ...request.lasmexEnv !== undefined ? { lasmexEnv: request.lasmexEnv } : {},
        sandboxPolicy: request.sandboxPolicy,
      }
    }
  ...
  async function setupRecording(withJsonl = false) {
    ...
    await ctx.plugin(LocalJobRegistry)
    await ctx.plugin(ToolTasks)
    await ctx.plugin(BashEnvPlugin, { lasmexHome: recordingLasmexHome })
  ...
  it('describes the managed harness environment namespace to the model', async () => {
    ...
    expect(description).toContain('$LASMEX_*')
    expect(description).not.toContain('LASMEX_SESSION_JSONL')
  })
  ...
  it('injects the session id and JSONL target path into a foreground request', async () => {
    ...
    expect(bash.requests[0]?.lasmexEnv).toEqual({
      LASMEX_HOME: recordingLasmexHome,
      LASMEX_SESSION_ID: 'request-fg',
      LASMEX_SESSION_JSONL: path,
      LASMEX_SHELL: '1',
    })
  })
  ...
  it('injects the same trusted variables into a background request without forwarding model env', async () => {
    ...
    await ctx.tools.execute({
      ...
      arguments: {
        ...
        env: { LASMEX_SESSION_ID: 'spoofed', LASMEX_SESSION_JSONL: '/tmp/spoofed' },
      },
      agent,
    })
  ...
  it('injects built-ins and the stable session id when no JSONL locator is available', async () => {
    ...
    const ambient = process.env.LASMEX_SESSION_ID
    ...
    expect(bash.requests[0]?.lasmexEnv).toEqual({
      LASMEX_HOME: recordingLasmexHome,
      LASMEX_SESSION_ID: 'request-id-only',
      LASMEX_SHELL: '1',
    })
    expect(process.env.LASMEX_SESSION_ID).toBe(ambient)
  })
  ...
  it('keeps parent and child agent session environments isolated', async () => {
    ...
    expect(bash.requests.map(request => request.lasmexEnv)).toEqual([
      {
        LASMEX_HOME: recordingLasmexHome,
        LASMEX_SESSION_ID: 'request-parent',
        LASMEX_SESSION_JSONL: ctx.sessionPersistence.locate(parent.session.header)?.path,
        LASMEX_SHELL: '1',
      },
      {
        LASMEX_HOME: recordingLasmexHome,
        LASMEX_SESSION_ID: 'request-child',
        LASMEX_SESSION_JSONL: ctx.sessionPersistence.locate(child.session.header)?.path,
        LASMEX_SHELL: '1',
      },
    ])
    expect(bash.requests[0]?.lasmexEnv?.LASMEX_SESSION_JSONL).not.toBe(bash.requests[1]?.lasmexEnv?.LASMEX_SESSION_JSONL)
  })
