...
import { SettingsProvider, deepEqualJson, type SettingsNamespace } from 'lasmex-settings'
...
export function resolveSpec(config: Config): ResolvedSpec {
  const filename = resolve(config.path ?? join(resolveLasmexHome(config.lasmexHome), 'settings.yaml'))
