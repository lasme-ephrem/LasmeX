 ...
 *
 * @module lasmex-sdk-jsonrpc-demo/runner
 */
...
const NAME = 'lasmex-jsonrpc-agent'
...
export async function runJsonrpcAgent(bareModuleBaseUrl?: string): Promise<void> {
  ...
  const fromEnv = process.env['LASMEX_CORDIS_CONFIG']
  ...
  if (configPath === undefined || !existsSync(configPath)) {
    process.stderr.write(
      `usage: ${NAME} <path/to/cordis.yml> (or set LASMEX_CORDIS_CONFIG=<path>, which wins); the config is required — there is no built-in fallback\n`,
    )
    process.exit(1)
  }
