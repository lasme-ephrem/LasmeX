...
- **The bin cannot prove that the config serves JSON-RPC** — a valid config with no `lasmex-sdk-jsonrpc-server` entry boots successfully and serves nothing.
- **No built-in or default config exists** — every launch must provide `LASMEX_CORDIS_CONFIG` or a positional path, and deployment owns the complete plugin tree and stdout discipline.
