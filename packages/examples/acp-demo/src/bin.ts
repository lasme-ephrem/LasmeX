 ...
 * `lasmex-acp-demo [--config path]`, defaulting to `./cordis.yml`. Shared env
 * loading, Loader guards, snapshot config selection, and settled-tree boot live
 * in lasmex-app-boot. Replay skips `.env` and selects sibling
 ...
 * @module lasmex-acp-demo/bin
 */

import { parseArgs } from 'node:util'
import { boot, installFailLoud, loadEnv, resolveConfigPath } from 'lasmex-app-boot'

const NAME = 'lasmex-acp-demo'

/* v8 ignore start -- thin self-executing composition over the unit-tested
   lasmex-app-boot helpers; exercised end-to-end by the snapshot suite and the
   built-bin smoke */
