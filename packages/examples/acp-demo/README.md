...
ACP automation server app: the default agent spine, client-created agents through [`lasmex-acp`](../../acp/acp/README.md), JSONL persistence, and semantic checkpointing behind one JSON-RPC stdio bin. Programmatic clients create fresh sessions; this package mounts no human UI.

## Composition

| Plugin | Role |
|---|---|
| `lasmex-agent-spine-demo` | Providerless agent spine with no pre-created agents; `session/new` creates each agent. |
| `lasmex-session-persistence-jsonl` | Durable session logs used by checkpointing, observability, and snapshot replay. |
| `lasmex-session-checkpoint-policy` | Durability barriers before model calls and top-level tool effects, plus completed-step checkpoints. |
| `lasmex-session-query-sqlite` | Derived exact/FTS session-query service, opened before the ACP transport so leaf consumers are ready for the first model request. |
| `lasmex-acp` | Automation-only ACP transport over stdin/stdout. |
...
| `persona` | — | Deployment persona template for `lasmex-system-prompt`. |
| `toolOrder` | lexicographic | Explicit model-facing tool order for `lasmex-system-prompt`. |
| `tools` | `{ mode: 'native' }` | Native, Code Mode, or combined model tool transport. |
| `lasmexHome` | `$LASMEX_HOME` or `~/.lasmex` | Harness home shared by bash and local skill discovery. |
