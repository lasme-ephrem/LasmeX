...
lasmex-llm              abstract LLM service + content-block vocabulary
lasmex-session          event-sourced session log + store
lasmex-session-title    log-backed title service + deterministic fallback
lasmex-system-prompt    prompt-section + tool-schema assembly
lasmex-tools            registry + guarded pre/around/post/final-result pipeline
lasmex-skill            skill provider registry
lasmex-skill-filesystem      local filesystem skill provider
lasmex-agent            agent registry + initiator scope + agent/* events
lasmex-goal             optional persisted same-session goal domain
lasmex-tool-goal        optional model-facing goal controls
lasmex-goal-round-driver     optional same-session goal-round driver
lasmex-llm-retry        provider-routed request retry policy
lasmex-jobs-local      generic background-job registry
lasmex-invariants       configurable invariant registry service
lasmex-session/invariant
lasmex-agent/invariant
lasmex-scope/invariant
lasmex-agent-loop/invariant
                                  package-owned relational checks
lasmex-tool-bash        the model-facing bash schema (unless toolBash=false)
lasmex-agent-instructions  AGENTS.md/CLAUDE.md workspace context loader
lasmex-tool-skill       session-prefix skill catalog + model-facing loader schema
lasmex-tool-jobs       job_output/job_list/job_kill schemas + completion notices
lasmex-agent-loop       THE concrete loop (gets the forwarded `agents`)
                                  (lasmex-system-prompt gets the forwarded `persona`)
```
...
```ts
import type { Config } from 'lasmex-agent-spine-demo'
// { agents?, maxParallelToolCalls?, maxStepsPerTurn?, includeHarnessIdentity?, harnessIdentity?, includeRuntimeContext?, persona?, toolOrder?, tools?, lasmexHome?, sessionTitle?, skills?, workspaceContext, toolBash?, jobs?, toolJobs?, goals?, invariants? }
// workspaceContext requires { maxBytes } or false; the other owner schemas supply defaults.
```

The bundle forwards each field to the child that owns it. App packages supply any pre-created agents: headless and JSON-RPC compositions create `main`, while the ACP app creates agents on demand at `session/new`. `harnessIdentity` is forwarded to `lasmex-system-prompt` as the product identity opener, while `includeRuntimeContext: false` suppresses all dynamic context snapshots for fresh sessions without disabling their policy services. Prompt, tool, title, skill, agent-instructions, invariant, goal, and task settings retain the schemas and defaults documented by their owning packages; `jobs.maxConcurrentJobsPerOwner` configures the local provider independently of the model-facing `toolJobs` controls. `pickSpineConfig()` copies only fields owned by this bundle, and conflicting `lasmexHome` values fail during composition.

For example, `{ invariants: { enabled: true, package_allowlist: ['^lasmex-'], package_blocklist: ['agent-loop$'] } }` keeps the package-owned companions mounted but suppresses the blocked owner. Blocklist matches override allowlist matches; see [`lasmex-invariants`](../../runtime-diagnostics/invariants/README.md) for regex and lifecycle rules.
...
Indirectly, through `lasmex-system-prompt`, `lasmex-tool-skill`, `lasmex-tool-bash`, `lasmex-tools`, and `lasmex-llm-retry`, plus `lasmex-tool-goal` and goal-round prompts when `goals` is enabled. The bundle adds no model-bound wrapper content of its own.
