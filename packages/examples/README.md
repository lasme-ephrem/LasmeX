...
|---|---|---|
| [`agent-spine-demo/`](agent-spine-demo/README.md) | `lasmex-agent-spine-demo` | Reusable agent-spine bundle |
| [`acp-demo/`](acp-demo/README.md) | `lasmex-acp-demo` | ACP automation application bundle |
| [`jsonrpc-demo/`](jsonrpc-demo/README.md) | `lasmex-sdk-jsonrpc-demo` | External-config JSON-RPC runtime |

`agent-spine-demo` is the shared bundle; `acp-demo` adds its automation entry point, while `jsonrpc-demo` boots a deployment-owned plugin tree. Product one-shot execution belongs to `lasmex --profile headless`; no package in this directory provides it.
