...
Node 侧会扫描已启用的 Loader 配置项以发现 web `lasmex.client` 包，解析每个 `exports["./client"]`，把构建后的 bundle 哈希写入启动图，并通过 `/plugins` 提供该文件及其 sourcemap。包元数据优先从可写配置文件解析，然后从已安装的运行时解析：用户插件保持优先级，而封闭的 Electron 运行时无需通过指向 ASAR 归档的配置文件符号链接也能发现内置客户端包。源码启动会把宿主侧导入映射到 TypeScript 源码，但仍消费这一构建后的客户端导出；缺失文件共享一条构建说明，随后以包／路径列表列出各项，而无关的文件系统错误仍是独立故障。

Registry 与图不依赖 Web server，也能独立使用。挂载 `lasmex-host-webserver` 时，本包会把 `/plugins` route 与 index manifest 注入接到该服务；本地应用载体也可以直接读取 `clientPath(id)`、`graph()` 与共享的安全 JSON 序列化器，从而在不开放端口、不复制模块发现逻辑的情况下复用完全相同的已组合插件清单。
...
- **自身不维护卸载记录**：样式移除与 fiber 拆卸顺序属于 HMR 驱动器（`lasmex-client-hmr`）；loader 只在每条记录中登记其拥有的样式标签 id。
