...
import { stubSettingsScope, type StubSettingsScope } from 'lasmex-client-test-runtime'
import type { LocaleSettings, LocaleSnapshot } from 'lasmex-client-locale/client'
...
describe('LocaleRuntime', () => {
  ...
  it('translates through the active-locale -> French fallback -> key chain', () => {
    const { svc } = make()
    svc.register('ns', 'zh', { hello: '你好' })
    svc.register('ns', 'fr', { hello: 'Bonjour', onlyFr: 'Seulement en français' })
    ...
    expect(t('onlyFr')).toBe('Seulement en français')
  ...
  it('opens provisionally in the browser language, including French regional variants', () => {
  ...
  it('keeps the document language aligned with browser, user, and Host selections', () => {
    stubLanguages('en-US')
    const host = stubSettingsScope<LocaleSettings>()
    const { svc } = make(host)
    ...
    host.publish({ status: 'ready', value: { preference: 'fr' }, revision: 1, writable: true })
