...
afterEach(cleanup)
import { AppRoot } from 'lasmex-client-web/src/AppRoot.tsx'
import { createLoaderStatusStore, createSignal } from 'lasmex-client-web/src/loader-status.ts'
...
describe('AppRoot', () => {
  it('shows the loading page and never calls renderApp before settled', () => {
    ...
    expect(getByText('LasmeX')).toBeTruthy()
    expect(getByText('Chargement des plugins…')).toBeTruthy()
  ...
  it('lists failed entries and stays on the loading page', () => {
    ...
    act(() => {
      status.set('lasmex-client-ui-layout', 'failed')
      status.set('ok', 'active')
    })
    expect(getByText('Échec du chargement des plugins')).toBeTruthy()
    expect(getByText('lasmex-client-ui-layout')).toBeTruthy()
    expect(queryByTestId('real-ui')).toBeNull()
  })
  ...
  it('flipping settled switches to the real UI in one pass', () => {
    ...
    expect(queryByText('LasmeX')).toBeNull()
