...
describe('WorkspaceBrowser.module.css list', () => {
  ...
  it('counts the themed scrollbar inside the shell trailing inset', () => {
    expect(root?.get('--lasmex-session-list-edge-inset')).toBe('var(--lasmex-sidebar-inline-padding)')
    expect(root?.get('--lasmex-session-list-scrollbar-width')).toBe('8px')
    expect(root?.get('--lasmex-session-list-scrollbar-offset')).toBe('2px')
    expect(root?.get('padding-right')).toBe('var(--lasmex-session-list-edge-inset)')
    expect(listArea?.get('margin-left')).toBe('-4px')
    expect(listArea?.get('padding-left')).toBe('4px')
    expect(listArea?.get('margin-right')).toBe('calc(-1 * var(--lasmex-session-list-edge-inset))')
    expect(declarations('.fade')?.get('right')).toBe('var(--lasmex-session-list-edge-inset)')
    expect(list?.get('margin-right')).toBe('var(--lasmex-session-list-scrollbar-offset)')
    expect(list?.get('margin-left')).toBe('-4px')
    expect(list?.get('padding-left')).toBe('4px')
    expect(list?.get('padding-right')).toBe([
      'calc(',
      'var(--lasmex-session-list-edge-inset)',
      '- var(--lasmex-session-list-scrollbar-width)',
      '- var(--lasmex-session-list-scrollbar-offset)',
      ')',
    ].join(' '))
