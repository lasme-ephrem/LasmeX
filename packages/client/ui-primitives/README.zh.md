...
- **面向用户的文案经 label props 本地化，默认值为原文字面量**：这些原子组件是 zero-cordis 的，拿不到 `ctx.locale`，因此 `HoverCard`、`TerminalBlock`、`JsonTree`、`CodeBlock`、`MarkdownText`、`JsonBlock`、`ConnectionBanner`、`Modal` 以及结构化的 `DiffBlock`、`ReadBlock`、`SearchBlock` 和 `WebBlock` 卡片都把文案作为可选 props 接收。`MarkdownText` 还可接收脚注区域的辅助技术标题。已本地化的插件用自己的 `t` 席位传入字典驱动的 label；什么都不传的消费方得到的是原有默认值。
