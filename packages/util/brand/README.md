...
A package brands the ids it owns — `CallId` in `lasmex-llm`, the shared agent/session `SessionId` in `lasmex-session`, and `JobId` in `lasmex-jobs`. Brand cross-package ids that could plausibly be confused; not every string needs one.

This package owns only the primitive. Keeping it dependency-free lets `lasmex-jobs`, for example, brand `JobId` without importing an unrelated capability package merely to reach `Branded`.
