...
import * as GoalSessionInvariant from 'lasmex-goal-round-driver/invariant'
import { renderGoalRoundPrompt } from 'lasmex-goal-round-driver'
import InvariantRegistry, { InvariantError } from 'lasmex-invariants'
import SessionStore, { SessionId, type Session } from 'lasmex-session'
...
describe('goal-round-driver prompt invariants', () => {
  ...
  it('rejects a continuation whose content differs from the package renderer', async () => {
    ...
    expect(() => {
      ...
      code: 'INVARIANT',
      packageName: 'lasmex-goal-round-driver',
    }))
  })
