...
import LlmRuntime, { createUserMessage, LlmAdapter  } from 'lasmex-llm'
...
import * as providerPlugin from 'lasmex-session-title-first-prompt-llm'
...
async function loadComposition(): Promise<Context> {
  ...
  await writeFile(configPath, [
    "- name: 'lasmex-llm'",
    "- name: 'lasmex-session'",
    "- name: 'lasmex-session-title'",
    '  config:',
    '    fallbackMaxWords: 5',
    '    fallbackMaxBytes: 40',
    '    maxTitleBytes: 80',
    "- name: 'lasmex-session-title-first-prompt-llm'",
    '  config:',
    '    targetWords: 5',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['lasmex-session-title', SessionTitleService],
    ['lasmex-session-title-first-prompt-llm', providerPlugin],
  ])
