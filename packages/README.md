...
npm scope: `lasmex-*`; Cordis `Service` subclasses and function plugins contribute through `ctx.effect()`, `ctx.on()`, or `ctx.waterfall()`. Rules: [package](AGENTS.md), [root](../AGENTS.md#conventions).

## Hierarchy

Groups hold `packages/<group>/<pkg>/`; names stay `lasmex-<pkg>`. **Group READMEs own package/ctx-key maps.**
...
| [`bundle/`](bundle/README.md) | Installable `lasmex --profile` patch layers | Product — stable API |
...
| [`memory/`](memory/README.md) | Project-memory seam: Service Definition, durable storage-domain provider, and approval-aware tools | Product — stable API |
