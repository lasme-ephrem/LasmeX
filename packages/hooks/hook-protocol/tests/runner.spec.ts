...
import type { ShellExecRequest, ShellExecSpec, ShellExecutor, ShellRunResult } from 'lasmex-shell'
import { DEFAULT_HOOK_TIMEOUT_MS, runHook } from 'lasmex-hook-protocol'
import type { RunHookOptions } from 'lasmex-hook-protocol'
 ...
 * executor (lasmex-bash-local) is exercised end-to-end by the hook-bridge plugins
...
function recordingBash(run: (spec: ShellExecSpec) => Promise<ShellRunResult>): {
  ...
  const bash = {
    resolve(request: ShellExecRequest): ShellExecSpec {
      ...
      // exactly what lasmex-bash-local's resolve does for the fields runHook sets.
      return {
