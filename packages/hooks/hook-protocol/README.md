# lasmex-hook-protocol

English | [中文](README.zh.md)

The **shared core** of the Claude Code / Codex hook wire protocol. NOT a cordis plugin — it registers nothing and injects nothing. It is a **library** of dialect-neutral primitives the two bridge plugins (`lasmex-hooks-claude-code`, `lasmex-hooks-codex`) import so neither re-implements the identical halves of the protocol.
...
| Concern | Here (`lasmex-hook-protocol`) | The bridge (`lasmex-hooks-claude-code` / `-codex`) |
|---|---|---|
...
Indirectly, through `lasmex-hooks-claude-code` and `lasmex-hooks-codex`, which can turn parsed hook output into prompt context, blocked outcomes, or continuation feedback.
