...
LasmeX SDK 运行时的共享协议格式（wire format）：一个按换行分帧的 JSON-RPC 2.0 传输类，加上协议两端共同使用的具名请求、结果与通知类型。包根枚举协议消费方接口；源模块不支持深层导入。服务端是 [`lasmex-sdk-jsonrpc-server`](../server/README.md) 插件；客户端是 [`lasmex-sdk-client`](../client/README.md)（TypeScript）与 [Python SDK](../../../python/README.md)（后者复现这些结构但不导入它们）。纯库——无插件、无 Config、无注册。
...
- **无取消与会话关闭方法**——客户端放弃轮次的方式是关闭运行时进程；见 [`lasmex-sdk-jsonrpc-server` README](../server/README.md)。
