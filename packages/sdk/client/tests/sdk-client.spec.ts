...
function harnessWith(env: Record<string, string> = {}, extra: LaunchOverrides = {}): LasmeX {
  const harness = new LasmeX({ launch: fakeLaunch(env, extra) })
...
describe('LasmeX', () => {
  ...
  it('resolves a relative launch cwd to an absolute workspace before the handshake', async () => {
    ...
    const dir = await mkdtemp(join(process.cwd(), '.lasmex-sdk-client-relcwd-'))
    cleanups.push(() => rm(dir, { recursive: true, force: true }))
    const recordFile = join(dir, 'init.jsonl')
  ...
  it('supports await using disposal', async () => {
    let captured: LasmeX
    {
      await using harness = new LasmeX({ launch: fakeLaunch() })
      captured = harness
...
describe('HarnessClient', () => {
  ...
  it('delivers notifications to unfiltered and filtered subscriptions in wire order', async () => {
    ...
    expect(identity.serverInfo.name).toBe('lasmex-sdk-runtime')
