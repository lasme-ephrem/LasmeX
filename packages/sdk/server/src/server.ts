 ...
 *
 * @module lasmex-sdk-jsonrpc-server/server
 */

import type { Context } from '@deepseek-ai/cordis'
import { resolve } from 'node:path'
import type { Agent, AgentHandle } from 'lasmex-agent'
import { createUserMessage } from 'lasmex-llm'
import { carrierKeyOf, type Scoped } from 'lasmex-scope'
import { SessionId } from 'lasmex-session'
import type SubagentRuntime from 'lasmex-subagent'
import type { SubagentRunEndInfo } from 'lasmex-subagent'
...
export class HarnessSdkJsonRpcServer {
  ...
  async initialize(params: InitializeParams): Promise<InitializeResult> {
    ...
    return { serverInfo: { name: 'lasmex-sdk-runtime', version: '0.0.1' } }
  }
  ...
  async handleRequest(method: string, params: Record<string, unknown> | undefined): Promise<unknown> {
    switch (method) {
      ...
      default:
        throw new Error(`unknown LasmeX SDK runtime method: ${method}`)
    }
  }
