...
两个插件都有可用默认值。meter 保持与模型路由和可选压缩无关。部署会在 LLM（大语言模型）适配器上配置容量，并在 `lasmex-compaction-basic` 上配置压缩策略。

## 模型体验

通过 `lasmex-compaction-basic` 等消费方间接影响；该服务自身不添加提示词、消息、schema、工具或模型调用。
