...
import LlmRuntime, { createUserMessage, LlmAdapter, LlmError, resolveRetryPolicy  } from 'lasmex-llm'
import type { GenerateOptions, ResolvedRetryPolicy, StreamChunk } from 'lasmex-llm'
...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'lasmex-llm-retry-loader-'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['lasmex-llm-retry', retry],
    ['lasmex-agent-loop', AgentLoop],
  ])
