/**
 * lasmex-llm's owned branded ids: tool-call correlation and provider request
 * diagnostics.
 *
 * The `Branded<B>` primitive itself lives in `lasmex-brand` (a
 * zero-dependency type-only package) so every owner of a cross-boundary id can
 * brand it without depending on lasmex-llm; see that package's README for the
 * nominal-typing policy.
 *
 * @module lasmex-llm/brand
 */
