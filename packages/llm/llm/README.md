...
- `EMPTY_RESPONSE_CODE` — the provider-neutral code both adapters use for a degenerate provider completion: a terminal `stop` that carried no content blocks at all. Classified as an error finish (not a successful empty message) because the attempt produced nothing durable; `lasmex-llm-retry` retries it by default.
...
Two adapters implement `LlmAdapter` on different internals: [`lasmex-llm-deepseek`](../llm-deepseek) uses direct fetch with `eventsource-parser` SSE framing for the `deepseek-official` route, while [`lasmex-llm-pi-ai`](../llm-pi-ai) dynamically resolves configured provider/model pairs through `@earendil-works/pi-ai`. Both follow the `StreamChunk` conventions in `types.ts`: usage precedes finish and tool arguments remain raw strings. Adapter implementations may throw or emit a failure finish internally; `LlmRuntime` exposes both as a terminal failure finish. See [the twin LLM adapters](../../../.agents/notes/implemented/architecture/2026-06-13-twin-llm-adapters.md) for the adapter rationale and [the terminal-failure decision](../../../.agents/notes/implemented/architecture/2026-07-29-terminal-llm-stream-failures.md) for the service boundary.
...
- **No retry execution, caching, or rate limiting ships in this service** — provider registration stores retry policy, but `llm/stream` remains a single-attempt call wrapper. The agent loop separately offers proven model-request failures to `agent/request-error`, whose default preserves the original failure; `lasmex-llm-retry` is the optional executor loaded by the shared example spine.
...
- **Official LasmeX source home** — `APP_IDENTITY.url` is `https://github.com/lasme-ephrem/LasmeX`, so provider requests identify the product, version, and public fork without presenting the DeepSeek Harness upstream as the LasmeX home.
- **`GenerateOptions.sessionId` is a locally-declared brand** — importing lasmex-session's `SessionId` would cycle; a future ids-owning package would dissolve the workaround.
