...
- **No cross-home identity** — different `$LASMEX_HOME` values cannot be correlated.
- **Configured DeepSeek gateways receive the id** — `lasmex-llm-deepseek` sends the stable header to its resolved `baseURL`, including deployment overrides, independently of telemetry sharing mode.
