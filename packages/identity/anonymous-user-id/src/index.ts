 ...
 * harness home resolved by {@link resolveLasmexHome} (`$LASMEX_HOME` > `~/.lasmex`),
 ...
 * machine: every process sharing one `$LASMEX_HOME` reports the same id, and
 ...
 *
 * @module lasmex-anonymous-user-id
 */
...
export interface AnonymousUserIdOptions {
  /** Environment consulted for `LASMEX_HOME`; defaults to `process.env`. */
...
export function getOrCreateAnonymousUserId(options: AnonymousUserIdOptions = {}): AnonymousUserId {
  const file = join(resolveLasmexHome(undefined, options.env ?? process.env), ANONYMOUS_USER_ID_FILE_NAME)
