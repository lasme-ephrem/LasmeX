 ...
 *
 * @module lasmex-agent-instructions/files
 */

import { createReadStream } from 'node:fs'
import { stat } from 'node:fs/promises'
import { dirname, isAbsolute, join, relative, resolve } from 'node:path'
import type { FileSystem, FsInfo, FsTarget, FsVersion } from 'lasmex-fs'
import { assertNever } from 'lasmex-llm'
import { lasmexHomeDisplay } from 'lasmex-home-paths'
...
async function discoverInstructionFiles(
  ...
  const userGlobal = join(config.lasmexHome, USER_GLOBAL_FILE)
  ...
  switch (userGlobalProbe.kind) {
    case 'present':
      addFile({
        ...
        displayPath: userGlobalDisplayPath(config.lasmexHome),
...
export async function probeScopeInstruction(
  ...
  const file: ProbedInstructionFile = {
    ...
    displayPath: directory === USER_GLOBAL_DIRECTORY
      ? userGlobalDisplayPath(resolved.lasmexHome)
...
function userGlobalDisplayPath(lasmexHome: string): string {
  return `${lasmexHomeDisplay(lasmexHome)}/AGENTS.md`
}
