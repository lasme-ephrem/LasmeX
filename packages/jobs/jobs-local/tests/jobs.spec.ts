...
import { bindScopeParent, createScope, scopeOf } from 'lasmex-scope'
import type { ScopeKey } from 'lasmex-scope'
import { JobId } from 'lasmex-jobs'
import type { JobHooks, JobKind, JobOutcome, JobSnapshot, JobStart } from 'lasmex-jobs'
...
describe('LocalJobRegistry.start', () => {
  ...
  it('refuses to register while no job controller serves the owner', async () => {
    ...
    expect(() => ctx.jobs.start(producer().spec))
      .toThrow('background jobs unavailable: no job controller serves this agent (load lasmex-tool-jobs in its composition)')
  })
