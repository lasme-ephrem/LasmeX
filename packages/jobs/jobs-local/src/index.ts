 ...
 * @module lasmex-jobs-local
 */

import { Context } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import type { Agent } from 'lasmex-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from 'lasmex-scope'
import type { ScopeLayer } from 'lasmex-scope'
import { deadline, timeoutOf } from 'lasmex-timeout'
import { JobRegistry, JobId } from 'lasmex-jobs'
import type {
 ...
 * `lasmex-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load lasmex-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load lasmex-agent)')
    }
