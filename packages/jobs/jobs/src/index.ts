 ...
 * process-local registry lives in `lasmex-jobs-local`.
 * @module lasmex-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('lasmex-jobs is the abstract job registry seam; load an implementation such as lasmex-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
