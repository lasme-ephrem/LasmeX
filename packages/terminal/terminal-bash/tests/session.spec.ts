...
import { LocalPtySession } from 'lasmex-terminal-bash/src/session.ts'
import type { ResolvedConfig } from 'lasmex-terminal-bash/src/config.ts'
import type { TerminalSendOperation, TerminalSessionStatus, TerminalSignal } from 'lasmex-terminal'
import type {
  SubprocessOutcome,
  SubprocessTerminalHandle,
  SubprocessTerminalSignal,
} from 'lasmex-subprocess'
import { TerminalError } from 'lasmex-terminal'
import type {
  ProcessIdentity,
  ProcessInspector,
} from 'lasmex-subprocess-local/src/process-inspector.ts'
...
async function initialize(session: LocalPtySession, terminal: FakeTerminal): Promise<void> {
  ...
  terminal.emitData('\x1b]133;D;0\x07lasmex> ')
...
describe('LocalPtySession readiness and output', () => {
  ...
  it('cancels with foreground-group SIGINT, observes AbortSignal, and contains write failures', async () => {
    ...
    terminal.emitData('\x1b]133;D;130\x07lasmex> ')
    await vi.advanceTimersByTimeAsync(10)
  ...
  it('does not attribute a delayed prior prompt to the current send', async () => {
    ...
    terminal.emitData('\x1b]133;D;0\x07lasmex> printf \'PID=%s\\n\' "$!"\r\n')
    await vi.advanceTimersByTimeAsync(20)
    expect(settled).toBe(false)

    terminal.emitData('PID=123\r\n\x1b]133;D;0\x07lasmex> ')
    await vi.advanceTimersByTimeAsync(10)
