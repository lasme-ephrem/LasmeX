...
import { normalizeTerminalText, TerminalSanitizer } from 'lasmex-terminal-bash/src/sanitize.ts'
...
describe('TerminalSanitizer', () => {
  it('removes split CSI and owned OSC prompt markers', () => {
    ...
    expect(sanitizer.push('D;0\x07lasmex> ')).toEqual({ text: 'lasmex> ', prompt: true, promptTail: 'lasmex> ' })
  })
  ...
  it('reports printable prompt text that follows a marker in a later chunk', () => {
    ...
    expect(sanitizer.push('lasmex> ')).toEqual({ text: 'lasmex> ', prompt: false, promptTail: 'lasmex> ' })
  })
