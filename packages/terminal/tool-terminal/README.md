...
The six generated schemas are listed in the [`lasmex-tool-terminal` catalog section](../../../docs/tool-catalog.md#lasmex-tool-terminal). Their fixed schema tokens are present whenever this plugin is active; agent-scoped tool filtering may hide them.
...
- Background mode requires both `lasmex-jobs` and its model-facing controller.
