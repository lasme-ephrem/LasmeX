 ...
 *
 * @module lasmex-acp-snapshot/harness
 */
...
export interface RunOptions {
   ...
   * them to `lasmex-llm-replay` via `$DSH_SNAPSHOT_CHILD_FILES` so each child
...
export function snapshotSpillRoot(
  ...
  return `${root}/lasmex-acp-snap-${key}`
}
...
async function harvestSessionLogs(root: string): Promise<HarvestedLog[]> {
  ...
  // order must match the replay load order in lasmex-llm-replay's loadSessionScripts
