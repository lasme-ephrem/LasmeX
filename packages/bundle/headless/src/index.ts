/**
 * lasmex-headless — one-shot direct Agent driver. The bundle patch
 * rides over lasmex-base without Host, HTTP, or browser plugins; this runner
...
import type { ModelSelectionRef } from 'lasmex-agent'
import type {} from 'lasmex-agent-default-model'
...
import type {} from 'lasmex-cmdline'
...
function fail(io: HeadlessIo, error: unknown): void {
  io.stderr.write(`LasmeX : ${error instanceof Error ? error.message : String(error)}\n`)
  io.exit(1)
}
...
async function run(ctx: Context, task: string, io: HeadlessIo): Promise<void> {
  ...
  if (outcome.reason?.kind === 'error') {
    io.stderr.write(`LasmeX : ${outcome.reason.error.code} : ${outcome.reason.error.message}\n`)
  }
...
export function apply(ctx: Context, config: Config): void {
  ...
  if (exit === undefined) {
    throw new Error('headless-runner : le lanceur doit fournir ctx.appExit avant le montage de l’arbre')
  }
