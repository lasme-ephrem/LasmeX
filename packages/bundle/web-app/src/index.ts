/**
 * lasmex-web-app — the browser-surface bundle's runtime glue plugin
 * plus the bundle patch (`cordis.patch.yml`, declared by the `lasmex.bundle.patch`
...
import { addHarnessSourceSection } from 'lasmex-app-boot'
import * as FrontendStatic from 'lasmex-host-frontend-static'
...
/** This LasmeX installation's root, from either this package's source or built entry. */
...
const LASMEX_WEB_URL = 'LASMEX_WEB_URL' as const
...
/** Model-visible orientation and acceptance boundary for sessions created through `lasmex web`. */
function webSurfacePrompt(webUrl: string): string {
  const updateContract = 'The client-plugin HMR receiver is active, but client-plugin changes reload without a refresh only while '
    + '`pnpm run dev:web` is also running from this same checkout to rebuild their bundles; verify that watcher before promising automatic updates. '
    + 'Every other change — the apps/web shell and plain packages — requires rebuilding the affected Web artifacts and verifying this existing URL after a page refresh. '
  return `You are interacting with the user through the LasmeX Web GUI at ${webUrl}. `
    + 'When the user refers to "this page", "this GUI", or "this app" without naming another target, they mean this GUI. '
    + 'The browser provides no implicit DOM, route, or screenshot context. '
    + updateContract
    + 'Starting another server does not update this GUI. '
    + 'The apps/web Vite entry builds the shell but is not a standalone application because only lasmex web injects window.__DSH_BOOT__. '
...
function localWebUrl(ctx: Context): string {
  ...
  if (port === undefined) throw new Error('web-app : service webServer absent pendant la résolution du runtime Web')
...
function resolveDistIndex(): string {
  ...
  try {
    return require.resolve('lasmex-web-frontend/dist/index.html')
  } catch {
    /* v8 ignore next 2 -- reachable only on a checkout without a built dist; the test tree builds it */
    throw new Error('web-app : le frontend de distribution n’est pas construit ; exécutez d’abord pnpm run build depuis la racine du dépôt')
  }
}
  ...
  if (config.surfaceContext) {
    ...
    ctx.inject(['shellEnv'], (runtimeCtx) => {
      runtimeCtx.shellEnv.register({
        name: 'web-runtime',
        variables: {
          [LASMEX_WEB_URL]: { description: 'Canonical local URL of the LasmeX Web GUI serving this session.' },
        },
        resolve: () => ({ [LASMEX_WEB_URL]: localWebUrl(runtimeCtx) }),
      })
    })
  }
  if (config.printUrl) {
    ...
    const printUrl = (): void => {
      ...
      console.log(`lasmex web: ${localWebUrl(ctx)}${lanCandidate === undefined ? '' : ` (LAN: http://${lanCandidate}:${String(port)})`}`)
    }
