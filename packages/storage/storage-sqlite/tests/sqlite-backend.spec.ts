...
import Storage, { storageBackendServiceKey } from 'lasmex-storage'
import type { KvUnitDescriptor } from 'lasmex-storage'
...
async function freshDbPath(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'lasmex-storage-sqlite-'))
  dirs.push(dir)
