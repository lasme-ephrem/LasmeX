 ...
 * (like `lasmex-bash-local`'s `run.ts`) so the filesystem behavior is unit-testable
 * without a `ctx` and without the OS temp dir.
 *
 * @module lasmex-spill-local/store
 */
...
export function privateRoot(): string {
  defaultRoot ??= mkdtempSync(join(tmpdir(), 'lasmex-spill-'))
  return defaultRoot
}
