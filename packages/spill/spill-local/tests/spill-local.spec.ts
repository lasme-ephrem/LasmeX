...
import type { SaveTextSpill } from 'lasmex-spill'
import LocalSpillStore, { encodeSegment, privateRoot, saveTextFile, sessionDir } from 'lasmex-spill-local'

let root: string

beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'lasmex-spill-test-'))
})
afterEach(() => {
