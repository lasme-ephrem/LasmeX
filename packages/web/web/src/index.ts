...
export class WebRuntime extends Service {
   ...
   * `$LASMEX_WEB_SEARCH_PROVIDER` / `$LASMEX_WEB_FETCH_PROVIDER` are equivalent to
  ...
  constructor(ctx: Context, config: WebRuntimeConfig = {}) {
    super(ctx, 'web')
    this.searchProviderId = config.searchProvider ?? process.env.LASMEX_WEB_SEARCH_PROVIDER
    this.fetchProviderId = config.fetchProvider ?? process.env.LASMEX_WEB_FETCH_PROVIDER
  }
