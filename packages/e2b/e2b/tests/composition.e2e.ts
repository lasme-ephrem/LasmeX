...
import { Inbox } from 'lasmex-agent'
...
import TerminalSessionService, { TerminalSessionId } from 'lasmex-terminal'
...
describe.skipIf(!process.env.E2B_API_KEY)('E2B live Loader composition', () => {
  it('scrubs credentials before actual E2B command and PTY login shells', async () => {
    ...
    try {
      const profileLeakPath = '/home/user/lasmex-e2b-bootstrap-profile-leak'
      ...
      ctx.provide('e2b', {
        cwd: '/home/user',
        runtimeRoot: '/home/user/.lasmex-e2b',
      ...
      const result = await session.startSend({
        text: "printf 'NPM=<%s> LasmeX=<%s> KEEP=<%s>\\n' \"$NPM_TOKEN\" \"$DSH_STALE\" \"$KEEP\"",
        submit: true,
      }).done
      expect(result.viewport).toContain('NPM=<> LasmeX=<> KEEP=<visible>')
  ...
  it('runs FS, Bash, PTY, and LSP in one sandbox and deletes it', async () => {
    const { stdout, stderr } = await runLoaderSmoke({
      ...
      tempDirPrefix: 'lasmex-e2b-composition-',
      binScript,
    ...
    expect(terminalMotd).not.toContain('.lasmex-e2b/terminals/')
