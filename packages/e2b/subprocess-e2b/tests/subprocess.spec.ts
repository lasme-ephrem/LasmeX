...
import type { SubprocessSpawnSpec } from 'lasmex-subprocess'
import E2BSubprocessRuntime from 'lasmex-subprocess-e2b'
...
class FakeSandbox {
  ...
  alive = true
  zombieOnly = false
  ambient = 'PATH=/ambient/bin\0KEEP=safe\0UNICODE=你好\0NPM_TOKEN=secret\0LASMEX_STALE=old\0BROKEN\0=bad\0'
...
describe('E2BSubprocessHandle', () => {
  it('starts asynchronously, keeps secrets out of the command, and supports deferred piped stdin/output', async () => {
    ...
    }), '/workspace/.lasmex-e2b/processes/one')
    ...
    expect(controlEnvs?.HOME).toMatch(/^\/\.lasmex-e2b-control-/)
    ...
    expect(command).not.toContain('LASMEX_MODE')
    ...
    expect(fake.writtenFiles[0]).toEqual([
      '/workspace/.lasmex-e2b/processes/one/pid',
      '/workspace/.lasmex-e2b/processes/one/exit-code',
      '/workspace/.lasmex-e2b/processes/one/environment',
      '/workspace/.lasmex-e2b/processes/one/stderr.log',
    ])
    expect(fake.writtenFileData.get('/workspace/.lasmex-e2b/processes/one/environment')).toBe(
      'PATH=/bin\0UNICODE=你好\0HOME=/home/user\0FOO-BAR=hyphen-value\0--split-string=literal-value\0DEEPSEEK_API_KEY=explicit-secret\0LASMEX_MODE=test\0',
    )
    ...
    expect(fake.removed).toContain('/workspace/.lasmex-e2b/processes/one/stderr.log')
