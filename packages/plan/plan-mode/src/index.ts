...
import type { Session, SessionEvent, UserMessage } from 'lasmex-session'
  ...
  constructor(ctx: Context, config: PlanModeConfig = { section: '' }) {
    ...
    ctx.on('agent/pre-step', async (
      ...
      try {
        ...
        ctx.logger.warn('lasmex-plan-mode: failed to append selected plan mode at step start: %o', error)
        return decision
      }
    ...
    ctx.effect(() => () => { disposed = true }, 'lasmex-plan-mode: close service lifetime')
