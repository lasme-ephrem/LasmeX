...
import type { TypertContext, TypertLookup } from 'lasmex-typert-protocol'
...
declare module '@deepseek-ai/cordis' {
  interface Context {
     ...
     * `lasmex-scope` tag while retaining it, so this field is DX context rather
...
export interface CreateAgentOptions {
   ...
   * {@link CreateSessionOptions.meta} in lasmex-session (the internal-only
 ...
 * via {@link AgentRegistry.setFactory}. Kept on the `lasmex-agent` interface so
 * consumers (e.g. the ACP bridge) program against `ctx.agents` without
 * depending on the concrete `lasmex-agent-loop` package.
 */
 ...
 * (`lasmex-agent-loop`), registered via {@link setFactory}.
 *
  ...
  constructor(ctx: Context) {
    ...
    ctx.inject(['typert'], (typeCtx) => {
      typeCtx.typert.lookups.register('agent', {
        parameter: 'agent',
        wire: 'agentId',
        hostTypeSymbol: 'lasmex-agent#Agent',
