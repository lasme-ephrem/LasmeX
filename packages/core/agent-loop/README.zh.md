...
可选的 `lasmex-agent-loop/invariant` 配套入口会向 `ctx.invariants` 注册请求重建。循环会把每个确切的冻结请求记录在 `lasmex-llm` 拥有的进程本地身份集合中；随后，配套入口要求存在实时会话，并根据日志独立重建消息边界和折叠后的请求 header。即使调用方冻结直接的一次性调用，或为其附加会话 id，这类调用仍不属于该约定。
...
通过配置创建的 agent 会自动启动。模型调用同时需要 `provider` 和 `model`；`agent/request` 可以在分发前补齐缺失的这一对值。可选的正数 `maxTokens` 会为每次对话请求提供初始输出上限，并记录在请求 header 中。`maxParallelToolCalls` 限制每个 agent 针对并行安全调用使用的滚动池，默认值为 `10`。`maxStepsPerTurn` 限制单个轮次中的模型请求数，默认值为 `64`；超过限制会以 `MAX_STEPS` 结束轮次。这两个字段都属于 `agent-loop` Settings 段，因此叠加在该条目之上的用户层无需重启即可改变下一次调度决策。非正整数会在写入时被拒绝，而不是等到下一组调用或步骤才失败。`agents` 刻意不在该段中——它在服务启动时被消费一次，所以存储的改动只会看起来生效。`cwd` 仅应用于全新会话，而 `resumeSessionId` 保留持久化元数据。通过配置创建的 agent 使用部署 persona；编程式 setup 可以按 agent 遮蔽它。该插件为每个 agent 提供 `provider`、`model` 和 `cwd` 提示词变量；harness 身份与部署 persona 属于 `lasmex-system-prompt`。
...
- 模型请求恢复：`lasmex-llm-retry` 在 `agent/request-error` 上记录并等待针对确切提供方配置的 normal 或无界退避，发出不进入表层的 `llm/retry` 状态，然后返回重试动作
...
- subagent：在循环外部实现为 `ctx.subagents` 提供方；进程内提供方使用 `ctx.agents.create()` 创建 agent，并通过其拥有的 `AgentHandle` 执行 teardown，而通用的 [`ctx.jobs`](../../jobs/jobs/) 与 [`lasmex-tool-subagent`](../../subagent/tool-subagent/) 负责后台收集。
...
已接纳的 user 消息、assistant 消息、工具调用与结果、注入上下文和 steering（中途引导）都会记录，并在后续步骤中发送。仅由结果触发的 continuation 会在工具历史之后增加一条插件 notice。它列出已完成工具的名称，禁止仅为满足被引用请求而重复调用，引用最近直接请求的文本块，并要求被指定为精确输出的响应保留标点。原始带标识的直接消息在历史中的较早位置保持不变；notice 不会复制非文本块。原始流分片、生命周期边界和其他仅写入日志的事件会被排除。

#### Token 影响

输入会随每条表层消息增长，直到压缩替换遮蔽较旧节点；包含多个步骤的工具轮次会在每个步骤重新发送累积的历史。每次仅由结果触发的 continuation 还会计入一条有界 notice，以及最近直接请求的重复文本。
