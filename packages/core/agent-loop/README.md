...
The optional `lasmex-agent-loop/invariant` companion registers request reconstruction with `ctx.invariants`. The loop records each exact frozen request in the process-local identity set owned by `lasmex-llm`; the companion then requires a live session and independently rebuilds the message boundary and folded request header from the log. Direct one-shot calls remain outside this contract even when callers freeze them or attach a session id.
...
interface Config {
  ...
  maxStepsPerTurn?: number       // default 64; positive safe integer
  agents: Array<{
    id: string                 // required
    provider?: string
    model?: string
...
Configured agents start automatically. A model call requires both `provider` and `model`; `agent/request` may supply a missing pair before dispatch. An optional positive `maxTokens` seeds each conversation request's output cap and is logged in its request header. `maxParallelToolCalls` bounds every agent's rolling pool for parallel-safe calls and defaults to `10`. `maxStepsPerTurn` bounds model requests in one turn and defaults to `64`; exceeding it ends the turn with `MAX_STEPS`. Both fields belong to the `agent-loop` Settings section, so a user layer over this entry can change the next scheduling decision without a restart. Values that are not positive integers are refused at the write rather than at the next group or step. `agents` is deliberately absent from that section — it is consumed once when the service starts, so a stored change could only look like it had an effect. `cwd` applies only to fresh sessions, while `resumeSessionId` retains persisted metadata. Configured agents use the deployment persona, and programmatic setup can shadow it per agent. This plugin supplies the per-agent `provider`, `model`, and `cwd` prompt variables; harness identity and deployment persona belong to `lasmex-system-prompt`.
...
The unified `send()` primitive routes content and source by (`target` × `wakeup`); `followup`/`steer`/`inject` are its fixed-preset aliases. `followup()` appends to the `next-turn` FIFO and wakes the driver, `steer()` appends to the `next-step` inbox and wakes it, and `inject()` appends to that same `next-step` inbox without waking it. At a turn boundary the driver opens the durable turn, then atomically claims pending next-step input plus one queued prompt; between steps it claims only next-step input. Claiming removes the batch through pure deletion splices and emits `agent/inbox/claimed { message, turn }` once per message. `agent/pre-step` then returns either rejection or the complete messages entering the proposed step. Before commit, the driver stably places every non-user-source message before every `source.kind === 'user'` message; both groups retain their relative order and identities, making the direct prompt the accepted batch's tail. When tool results alone require the next step, the loop appends a durable plugin notice that names every tool already called in the turn, tells the model to use those results without repeating the calls, and quotes the latest direct request so the remaining response instruction stays at the request tail. A new direct message in the accepted batch supersedes this automatic notice. Rejection leaves the claimed batch removed and closes the turn without a step; input inserted after the claim remains pending, and idle injection waits until follow-up or steering wakes the driver.
...
- Model-request recovery: `lasmex-llm-retry` records and waits exact-provider normal or unbounded backoff on `agent/request-error`, emits non-surface `llm/retry` status, then returns a retry action
...
- Sub-agents: implemented outside the loop as `ctx.subagents` providers; in-process providers use `ctx.agents.create()` and owned `AgentHandle` teardown, while generic [`ctx.jobs`](../../jobs/jobs/) plus [`lasmex-tool-subagent`](../../subagent/tool-subagent/) own background collection.
...
Accepted user messages, assistant messages, tool calls and results, injected context, and steering are logged and sent on later steps. A result-only continuation adds one plugin notice after the tool history. The notice lists completed tool names, prohibits repeats undertaken only to satisfy the quoted request, quotes the latest direct request's text blocks, and requires an exact requested response to preserve punctuation. The original identified direct message remains unchanged earlier in history; non-text blocks are not duplicated into the notice. Raw stream chunks, lifecycle boundaries, and other log-only events are excluded.

#### Token effect

Input grows with every surface message until a compaction replacement shadows older nodes; a multi-step tool turn resends the accumulated history each step. Each result-only continuation also pays for one bounded notice and the repeated text of the latest direct request.
