...
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from 'lasmex-scope'
import type { ScopeKey, ScopeLayer, Scoped } from 'lasmex-scope'
import type { CallId, ContentBlock, ToolSchema } from 'lasmex-llm'
import { assertNever, deepFreeze, HarnessError } from 'lasmex-llm'
import type { Agent } from 'lasmex-agent'
import { snapshotJsonValue } from 'lasmex-session'
import type { JsonValue, UserMessage } from 'lasmex-session'
import type { ToolProviderResult } from 'lasmex-system-prompt'
import type { CodeRuntime } from 'lasmex-code-runtime'
 ...
 * instead of deriving them: the seam's `lasmex-code-runtime` README pair, its
...
export type { JsonValue } from 'lasmex-session'
...
// lives in its own UI-facing module; re-export it so `lasmex-tools`
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('lasmex-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  /** Internal staged view consumed by `lasmex-agent-loop`'s parallel scheduler. */
  ...
  private sdkSection(): { name: string; order: number; text: (context: { scope?: ScopeKey }) => string } {
    return {
      ...
      text: (context) => {
        ...
        if (render === undefined) throw new Error(`lasmex-tools: no SDK renderer for ${runtime.language}`)
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`lasmex-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. lasmex-code-runtime-worker-thread) or set tools mode to "native"`)
    }
    if (!Object.hasOwn(SDK_RENDERERS, runtime.language)) {
      const known = Object.keys(SDK_RENDERERS).map(name => JSON.stringify(name)).join(', ')
      throw new Error(`lasmex-tools: no SDK renderer registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
    }
    return runtime
  }
   ...
   * `lasmex-agent-tool-presentation` exists for, and reading the deployment default would
