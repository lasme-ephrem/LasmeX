...
import { createUserMessage, CallId  } from 'lasmex-llm'
...
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from 'lasmex-tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from 'lasmex-tools'
import type { Agent } from 'lasmex-agent'
import { Session, SessionId } from 'lasmex-session'
import type { JsonValue, SessionEventMap } from 'lasmex-session'
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('lasmex-scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)
    ...
    // `lasmex-agent-tool-presentation` produces.
