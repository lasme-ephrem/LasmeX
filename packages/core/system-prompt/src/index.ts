 ...
 *
 * @module lasmex-system-prompt
 */

import { Context, Service } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeTarget } from 'lasmex-scope'
import type { ScopeKey, ScopeLayer, Scoped } from 'lasmex-scope'
import type { ContextSnapshotSection, ToolSchema } from 'lasmex-llm'
...
export class SystemPrompt extends Service {
  static Config: z<Config> = z.object({
    ...
    harnessIdentity: z.string().default('You are an AI agent powered by LasmeX.'),
  ...
  constructor(ctx: Context, config: Config) {
    ...
    if (config.includeHarnessIdentity ?? true) {
      this.section({
        ...
        order: -100,
        text: config.harnessIdentity ?? 'You are an AI agent powered by LasmeX.',
      })
    }
    this.section({
