...
| Key | Default | Meaning |
|---|---|---|
| `includeHarnessIdentity` | `true` | Include the order-−100 product identity. Set false only when a compatibility deployment owns the complete system prompt. |
| `harnessIdentity` | `You are an AI agent powered by LasmeX.` | Product identity delivered before the deployment persona when `includeHarnessIdentity` is true. |
