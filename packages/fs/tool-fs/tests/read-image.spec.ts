...
import { CodeRuntime } from 'lasmex-code-runtime'
import type { CodeRunRequest, CodeRunResult } from 'lasmex-code-runtime'
import { CallId, LlmAdapter, LlmRuntime } from 'lasmex-llm'
import type { GenerateOptions, LlmModelInfo, LlmResolvedModelInfo, StreamChunk } from 'lasmex-llm'
import SystemPrompt from 'lasmex-system-prompt'
import ToolRuntime, { RUN_CODE_NAME } from 'lasmex-tools'
import type { Config as ToolConfig } from 'lasmex-tools'
import LocalFileSystem from 'lasmex-fs-local'
import * as FsPolicy from 'lasmex-fs-observation-policy'
import LocalAttachmentStore from 'lasmex-attachment-local'
import { AttachmentId, AttachmentStore } from 'lasmex-attachment'
...
async function setup(options: SetupOptions = {}) {
  ...
  if (options.attachments !== false) {
    await ctx.plugin(LocalAttachmentStore, { lasmexHome: home, ...options.storeConfig })
  }
...
describe('registration surface', () => {
  it('withdraws read_image when the tool-fs fiber or the attachment store is disposed (HMR safety)', async () => {
    ...
    await ctx.plugin(LocalFileSystem, { cwd: dir })
    await ctx.plugin(FsPolicy)
    const attachmentsFiber = await ctx.plugin(LocalAttachmentStore, { lasmexHome: home })
    ...
    const remounted = await ctx.plugin(LocalAttachmentStore, { lasmexHome: home })
    expect(names()).toEqual(['edit', 'read', 'read_image', 'write'])
