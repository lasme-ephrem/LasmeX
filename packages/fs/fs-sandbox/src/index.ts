 ...
 * `lasmex-fs` Service Definition. It extends `LocalFileSystem` so all
 ...
 * stays `ctx.shell`'s job (`lasmex-bash-sandbox`). This mirrors the
 ...
 * escalation retry lives in the tool layer (`lasmex-tool-fs`),
 * exactly as bash's does.
 *
 * @module lasmex-fs-sandbox
 */

import { Context } from '@deepseek-ai/cordis'
import { LocalFileSystem } from 'lasmex-fs-local'
import type { Config as LocalConfig } from 'lasmex-fs-local'
import { FsError } from 'lasmex-fs'
import type { FsEditOutcome, FsEditRequest, FsTarget, FsVersion, FsWriteIntent, FsWriteOutcome } from 'lasmex-fs'
import { writableRoots } from 'lasmex-sandbox'
 ...
 * INSTEAD OF `lasmex-fs-local`, together with a `ctx.sandboxPolicy`, is the whole
 * swap — the model-facing tools are untouched). Its configured default mode is
 * the capability fact exposed by {@link sandboxMode}; `lasmex-tool-fs` resolves
