...
await ctx.plugin(LocalSubprocessRuntime)                     // lasmex-subprocess-local
...
await ctx.plugin(LocalSpillStore)                           // lasmex-spill-local
```
...
| `timeoutMs` | `30000` | Cooperative tool-call budget attached to both tool definitions, enforced by `lasmex-tool-call-timeout-policy` through `exec.signal`; the subprocess seam's terminate escalation is the hard kill. |
...
The glob description states the configured over-cap ordering. The generated [`glob` and `grep` schemas](../../../docs/tool-catalog.md#lasmex-tool-fs-search) use `sampleOverCapGlobResults: true`; the tools are registered unconditionally.
