 ...
 * model-facing render consumes ({@link module:lasmex-tool-fs-search/search-core}
 ...
 * final output budget (`lasmex-spill-policy`) only shrinks `content`, never `meta`,
 * so this projection owns keeping `meta` bounded.
 *
 * @module lasmex-tool-fs-search/presentation
 */
 ...
 * ({@link module:lasmex-tool-fs-search/grep} `formatGrepMatches`), so
