...
| Layer | Package | Role |
|---|---|---|
| tool / executor | `lasmex-tool-fs` | model-facing `read`/`write`/`edit` schemas + read windowing + text rendering; reads/writes/edits via `ctx.fs`, dispatches the `fs/*` events |
| policy | `lasmex-fs-observation-policy` | observed-state + read-before-edit + version-guarded write/edit, contributed through the `fs/*` event gate (no service) |
| provider contract | `lasmex-fs` (this) | `ctx.fs`: execution-world paths, text IO, and atomic mutation primitives (optional version guard); owns the `fs/*` event vocabulary |
| provider | `lasmex-fs-local` | the host-filesystem implementation |
...
Indirectly, through `lasmex-tool-fs`, which renders provider text and errors as bounded, retained filesystem tool results.
