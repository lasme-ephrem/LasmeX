...
import TypertRegistry from 'lasmex-typert-registry'
import type { TypertContribution } from 'lasmex-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from 'lasmex-tool-cordis/src/api-catalog.ts'
...
describe('model-driven LasmeX tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    const workspace = new WorkspaceAnalyzer({
      root: workspaceRoot,
      faces: ['host'],
      packages: ['lasmex-tools'],
    }).analyze()
    const host = workspace.faces.find(candidate => candidate.face === 'host')
    if (host === undefined) throw new Error('LasmeX tools have no analyzed host face')
    const artifact = new FaceModelEmitter(host).emit('lasmex-tools')
    ...
    const record = ctx.typert.getPackage('lasmex-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('lasmex-tools', 'host')).toBeUndefined()
  })
})
