# lasmex-typert-registry

[English](README.md) | 中文

生成的 Typert 产物所用的运行时注册表。每个注册项包含某个包在一个 face 上的业务反射信息，以及可选的运行时 Zod schema；`ctx.typert` 会以原子方式同时注册两者，并在发起调用的 Cordis fiber 释放时一并移除它们。TypeScript 分析和代码生成由 [`lasmex-typert-generator`](../generator/README.md) 负责。
...
`lasmex-typert-registry/types` 子路径包含注册项和记录的纯类型约定。[`lasmex-typert-loader`](../loader/README.md) 会在 Loader 组合中发现并注册生成的宿主侧产物；其他组合所有者可以直接调用 `ctx.typert.register()`。
