...
} from 'lasmex-typert-registry'
import type {
  InvocationDescriptor,
  TypertContext,
  TypertLookup,
  TypertRemoteContribution,
} from 'lasmex-typert-protocol'
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('lasmex-tools', 'ToolInput')).toBe('lasmex-tools#ToolInput')
    expect(typertPackageKey('lasmex-tools', 'host')).toBe('lasmex-tools#host')
    expect(ctx.typert.get('lasmex-tools#ToolInput')).toMatchObject({
      package: 'lasmex-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('lasmex-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('lasmex-tools', 'host')).toMatchObject({
      key: 'lasmex-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('lasmex-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('lasmex-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('lasmex-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('lasmex-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('lasmex-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('lasmex-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('lasmex-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('lasmex-tools#ToolInput')).not.toBe(projected)
  })
