/**
 * Profile machinery of `lasmex-app-boot`: directory resolution and init,
...
const tmp = (): string => mkdtempSync(join(tmpdir(), 'lasmex-profile-'))
...
function stageInstallation(bundles: Record<string, { patch?: string; deps?: Record<string, string> }>): string {
  ...
  for (const [name, spec] of Object.entries(bundles)) {
    ...
    writeFileSync(join(dir, 'package.json'), JSON.stringify({
      ...
      ...spec.patch === undefined ? {} : { lasmex: { bundle: { patch: './cordis.patch.yml' } } },
    }))
...
describe('resolveProfileDir', () => {
  it('joins the home and rejects traversal-shaped names', () => {
    ...
    for (const bad of ['', '.', '..', 'a/b', 'a\\b']) {
      expect(() => resolveProfileDir(bad, home)).toThrow('nom de profil invalide')
    }
  })
})
...
describe('initProfile', () => {
  it('creates manifest, user patch layer, and pnpm workspace once, never overwriting', () => {
    ...
    expect(readProfileManifest('t', dir).lasmex?.profile?.bundles).toEqual(['lasmex-base'])
...
describe('manifest round-trip', () => {
  it('writes and reads back, and fails loud on a broken manifest', () => {
    const dir = tmp()
    writeProfileManifest(dir, { name: 'p', lasmex: { profile: { bundles: ['a'] } } })
    expect(readProfileManifest('t', dir).lasmex?.profile?.bundles).toEqual(['a'])
    writeFileSync(join(dir, 'package.json'), '[]')
    expect(() => readProfileManifest('t', dir)).toThrow('doit contenir un objet JSON')
    expect(() => readProfileManifest('t', join(dir, 'nope'))).toThrow('lecture impossible du manifeste de profil')
  })
})
...
describe('resolveBundleDir', () => {
  it('prefers the installation anchor, falls back to the profile, and fails loud', () => {
    ...
    expect(() => resolveBundleDir('t', 'absent', anchor, profileDir)).toThrow('impossible de résoudre le bundle de profil')
  })
...
describe('loadProfile', () => {
  it('resolves each lasmex.profile.bundles entry to its patch layer in order, plus the user layer', () => {
    const anchor = stageInstallation({
    ...
    // A hand-made profile without the user layer file or lasmex section: empty layers, no throw.
  ...
  it('auto-initializes only shipped templates and fails loud otherwise', () => {
    ...
    expect(() => loadProfile('t', 'custom', anchor, home))
      .toThrow('le profil "custom" n\'existe pas')
    // The web template auto-initializes on first load. Bundle resolution
    // cannot be asserted to fail here: the source-plane test runner resolves
    // @deepseek-ai/* through tsconfig paths regardless of the staged anchor.
    expect(PROFILE_TEMPLATES.web).toContain('lasmex-base')
    try {
      loadProfile('t', 'web', anchor, home)
    } catch {
      // Resolution failure is the plain-Node outcome for this empty anchor.
    }
    expect(readProfileManifest('t', resolveProfileDir('web', home)).lasmex?.profile?.bundles)
  ...
  it('normalizes only the exact installation-owned headless bundle tuple', () => {
    const anchor = stageInstallation({
      'lasmex-base': { patch: '[]\n' },
      'lasmex-web-app': { patch: '[]\n' },
      'lasmex-headless': { patch: '[]\n' },
      'custom-bundle': { patch: '[]\n' },
    })
    const home = tmp()
    const stock = resolveProfileDir('headless', home)
    initProfile(stock, [
      'lasmex-base', 'lasmex-web-app', 'lasmex-headless',
    ])
    loadProfile('t', 'headless', anchor, home)
    expect(readProfileManifest('t', stock).lasmex?.profile?.bundles)
      .toEqual(['lasmex-base', 'lasmex-headless'])

    const customHome = tmp()
    const custom = resolveProfileDir('headless', customHome)
    initProfile(custom, [
      'lasmex-base', 'lasmex-web-app', 'lasmex-headless', 'custom-bundle',
    ])
    loadProfile('t', 'headless', anchor, customHome)
    expect(readProfileManifest('t', custom).lasmex?.profile?.bundles).toEqual([
      'lasmex-base', 'lasmex-web-app', 'lasmex-headless', 'custom-bundle',
    ])
  })

  it('fails loud when a listed bundle declares no lasmex.bundle', () => {
    const anchor = stageInstallation({ 'not-a-bundle': {} })
    const home = tmp()
    const dir = resolveProfileDir('demo', home)
    initProfile(dir, ['not-a-bundle'])
    expect(() => loadProfile('t', 'demo', anchor, home)).toThrow('ne déclare aucun lasmex.bundle')
  })
})
...
describe('healProfilesModuleFallback', () => {
  ...
  it('throws when a fallback entry is a real directory', () => {
    ...
    expect(() => { healProfilesModuleFallback(anchor, home) }).toThrow("n'est pas un lien symbolique")
  })
