 ...
 * `lasmex --profile` launcher family.
 *
 * A profile is a directory under `$LASMEX_HOME/profiles/<name>` holding a
 * `package.json` (out-of-tree plugin dependencies plus the profile manifest
 * `lasmex.profile` with its ordered `bundles` list) and a `cordis.patch.yml`
 * (the user's own patch layer, applied after every bundle layer). Bundles are
 * npm packages whose manifest declares
 * `"lasmex": { "bundle": { "patch": "./cordis.patch.yml" } }`; the tree is
 * composed by applying each bundle's patch list in `lasmex.profile.bundles` order over
 ...
 * first from the LasmeX installation (the launcher's own package), then from the
 ...
 * flat fallback directory `$LASMEX_HOME/profiles/node_modules` (one symlink per
 ...
 * @module lasmex-app-boot/profile
 */
...
import { resolveLasmexHome } from 'lasmex-home-paths'
...
/** The bundle half of the `lasmex` manifest section: what a bundle package exports. */
export interface LasmeXBundleManifest {
  /** The patch layer this bundle exports, relative to its package root. */
  patch: string
}

/** The profile half of the `lasmex` manifest section: what a profile directory composes. */
export interface LasmeXProfileManifest {
  /** Ordered bundle layer list (package names). */
  bundles?: string[]
}

/**
 * The profile-launcher slice of the `lasmex`-owned package.json section. A
 * manifest may declare both roles; other consumers own additional keys.
 */
export interface LasmeXManifestSection {
  /** Bundle metadata consumed by the profile launcher. */
  bundle?: LasmeXBundleManifest
  /** Profile metadata consumed by the profile launcher. */
  profile?: LasmeXProfileManifest
}
...
export interface ProfileManifest {
  ...
  lasmex?: LasmeXManifestSection
}

/** One resolved bundle layer of a profile. */
export interface ProfileLayer {
  /** The bundle's package name, as listed in `lasmex.profile.bundles`. */
  packageName: string
...
export interface Profile {
  ...
  dir: string
  /** Bundle layers in `lasmex.profile.bundles` order. */
  layers: ProfileLayer[]
 ...
 * @param name - the profile name (`lasmex --profile <name>`).
 * @param home - the Harness home; defaults to {@link resolveLasmexHome}.
 * @returns the absolute profile directory (which may not exist yet).
 */
export function resolveProfileDir(name: string, home: string = resolveLasmexHome()): string {
  if (name === '' || name.includes('/') || name.includes('\\') || name === '.' || name === '..'
    // The launcher-maintained flat module fallback lives at this sibling path.
    || name === 'node_modules') {
    throw new Error(`LasmeX : nom de profil invalide ${JSON.stringify(name)}`)
  }
...
export const PROFILE_TEMPLATES: Record<string, readonly string[]> = {
  web: ['lasmex-base', 'lasmex-web-app'],
  headless: ['lasmex-base', 'lasmex-headless'],
}

/** Installation-owned bundle tuples normalized to the shipped template. */
const INSTALLATION_OWNED_PROFILE_TUPLES: Record<string, readonly string[]> = {
  headless: ['lasmex-base', 'lasmex-web-app', 'lasmex-headless'],
}

/** The bundle list a `lasmex plugin` init uses for a name with no shipped template. */
export const DEFAULT_PROFILE_BUNDLES: readonly string[] = ['lasmex-base']

const PROFILE_PATCH_TEMPLATE = `# Your patch layer for this LasmeX profile, applied after every bundle layer:
 ...
 * @param bundles - the initial `lasmex.profile.bundles` layer list.
 */
export function initProfile(dir: string, bundles: readonly string[]): void {
  ...
  if (!existsSync(manifestPath)) {
    const manifest: ProfileManifest & { private: boolean } = {
      name: `lasmex-profile-${basename(dir)}`,
      private: true,
      dependencies: {},
      lasmex: { profile: { bundles: [...bundles] } },
    }
...
function ensureSymlink(link: string, target: string): void {
  ...
  if (stat !== undefined) {
    if (!stat.isSymbolicLink()) {
      throw new Error(`LasmeX : ${link} existe et n'est pas un lien symbolique ; supprimez-le pour que LasmeX puisse gérer le module de secours de l'installation`)
    }
...
/**
 * Maintain the flat module fallback `$LASMEX_HOME/profiles/node_modules`: one
 * symlink per package in the LasmeX app's resolvable dependency CLOSURE (BFS
 ...
 * Definition packages (`lasmex-compaction`, `lasmex-invariants`, ...) that the app
 ...
 * @param installAnchor - absolute path of the LasmeX app's package.json.
 * @param home - the Harness home; defaults to {@link resolveLasmexHome}.
 */
export function healProfilesModuleFallback(installAnchor: string, home: string = resolveLasmexHome()): void {
  ...
  for (let next = queue.shift(); next !== undefined; next = queue.shift()) {
    // Peer dependencies participate: Service Definition packages (lasmex-subprocess,
    // lasmex-compaction, ...) are peers of their implementations, never plain
...
export function readProfileManifest(binName: string, dir: string): ProfileManifest {
  ...
  try {
    ...
    throw new Error(`${binName} : lecture impossible du manifeste de profil ${path} : ${String(error)}`)
  }
  ...
  if (parsed === null || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new Error(`${binName} : le manifeste de profil ${path} doit contenir un objet JSON`)
  }
  return parsed
}
...
function normalizeShippedProfile(name: string, dir: string, manifest: ProfileManifest): ProfileManifest {
  ...
  const bundles = manifest.lasmex?.profile?.bundles
  ...
  const normalized: ProfileManifest = {
    ...
    lasmex: {
      ...manifest.lasmex,
      profile: { ...manifest.lasmex?.profile, bundles: [...current] },
    },
  }
 ...
 * `lasmex-base` (and every other in-box bundle) always comes from
 * the same installation as the running dsh, never from a profile-local copy.
 * Resolution does not require the package to export `./package.json`.
 * @param binName - the diagnostic prefix on the thrown error.
 * @param packageName - the bundle's package name from `lasmex.profile.bundles`.
 * @param installAnchor - absolute path of a file inside the LasmeX app package (its package.json).
...
export function resolveBundleDir(
  ...
  throw new Error(
    `${binName} : impossible de résoudre le bundle de profil ${JSON.stringify(packageName)} depuis l'installation LasmeX ou ${profileDir} ; `
    + `exécutez '${binName} plugin --profile ${basename(profileDir)} install' si sa dépendance n'est pas installée`,
  )
}

/**
 * Load a profile: resolve every `lasmex.profile.bundles` entry to its patch
 * layer and parse the profile's own patch file. A listed bundle without a
 * `lasmex.bundle` manifest fails loud — naming a bundle-less package as a layer
 * is a misconfiguration, not "no patches".
 * @param binName - the diagnostic prefix on thrown errors.
 * @param name - the profile name.
 * @param installAnchor - absolute path of the LasmeX app's package.json (first resolution anchor).
 * @param home - the Harness home; defaults to {@link resolveLasmexHome}.
...
export function loadProfile(
  binName: string, name: string, installAnchor: string, home: string = resolveLasmexHome(),
  ...
  if (!existsSync(join(dir, 'package.json'))) {
    ...
    if (template === undefined) {
      throw new Error(
        `${binName} : le profil ${JSON.stringify(name)} n'existe pas ; créez-le avec '${binName} plugin --profile ${name} add <package>'`,
      )
    }
    initProfile(dir, template)
  }
  const manifest = normalizeShippedProfile(name, dir, readProfileManifest(binName, dir))
  // A hand-written profile manifest may omit the lasmex section entirely.
  const bundles = manifest.lasmex?.profile?.bundles ?? []
  const layers = bundles.map((packageName): ProfileLayer => {
    const packageDir = resolveBundleDir(binName, packageName, installAnchor, dir)
    const bundleManifest = JSON.parse(readFileSync(join(packageDir, 'package.json'), 'utf8')) as ProfileManifest
    const declared = bundleManifest.lasmex?.bundle?.patch
    if (declared === undefined) {
      throw new Error(`${binName} : le bundle de profil ${JSON.stringify(packageName)} ne déclare aucun lasmex.bundle dans son package.json`)
    }
