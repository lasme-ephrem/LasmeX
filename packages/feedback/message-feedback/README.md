...
Public request, value, version, and failure types are exported from the package root and `lasmex-message-feedback/types`; [`src/types.ts`](src/types.ts) is their source.
