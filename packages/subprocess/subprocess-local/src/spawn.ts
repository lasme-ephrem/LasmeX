 ...
 * @module lasmex-subprocess-local/spawn
 */
...
function privateSpillDir(): string {
  defaultSpillDir ??= mkdtempSync(join(tmpdir(), 'lasmex-subprocess-'))
...
export class OutputCollector {
  ...
  private spillAll(chunk: Buffer): void {
    ...
    if (this.spillFd === undefined) {
      ...
      this.spillFile = join(
        this.spillDir,
        `lasmex-subprocess-${process.pid}-${++spillCounter}-${randomBytes(6).toString('hex')}-${this.label}.log`,
      )
