 ...
 * @module lasmex-subprocess/types
 */

import type { Readable, Writable } from 'node:stream'

/** Namespace prefix reserved for LasmeX-managed child environment facts. */
export const LASMEX_ENV_PREFIX = 'LASMEX_' as const

/** One environment key inside the managed {@link LASMEX_ENV_PREFIX} namespace. */
export type LasmexEnvironmentKey = `${typeof LASMEX_ENV_PREFIX}${string}`

/** Trusted LasmeX variables for one child-process execution. */
export type LasmexEnvironment = Readonly<Record<LasmexEnvironmentKey, string> & { LASMEX_HOME?: string }>
 ...
 * to `TerminalSignal` in `lasmex-terminal` without a cross-seam dependency;
