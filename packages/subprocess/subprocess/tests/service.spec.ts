...
import { scrubbedParentEnv, SubprocessRuntime } from 'lasmex-subprocess'
import type {
  SubprocessHandle,
...
describe('SubprocessRuntime seam', () => {
  ...
  it('scrubbedParentEnv drops credential-shaped and LASMEX_ names (case-insensitively) but keeps PATH', () => {
    process.env.LASMEX_SCRUB_PROBE = 'stale'
    process.env.lasmex_scrub_probe_lower = 'stale'
    process.env.SCRUB_PROBE_TOKEN = 'secret'
    process.env.SCRUB_PROBE_PASSWORD = 'secret'
    process.env.SCRUB_PROBE_PLAIN = 'visible'
    try {
      const env = scrubbedParentEnv()
      expect(env.LASMEX_SCRUB_PROBE).toBeUndefined()
      expect(env.lasmex_scrub_probe_lower).toBeUndefined()
      ...
      delete process.env.LASMEX_SCRUB_PROBE
      delete process.env.lasmex_scrub_probe_lower
