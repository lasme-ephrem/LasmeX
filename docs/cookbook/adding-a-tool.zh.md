...
中性词汇定义在 `lasmex-tools` 中；工具绝不导入 UI 或传输类型。host/client 运行时将每个 `card` 映射到各自的视图。设计与原因见[渲染意图联合体 Agent Note](../../.agents/notes/implemented/architecture/2026-07-02-tool-render-intent-union.md)；`lasmex-tool-fs`（generic/diff）和 `lasmex-tool-bash`（terminal）是参考实现。
