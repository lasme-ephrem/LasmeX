...
# Catalogue de configuration des plugins

Ce catalogue recense tous les blocs `config:` qu’une entrée de `cordis.yml` peut définir. Pour chaque package chargeable du harness, il reproduit mot pour mot la déclaration de configuration, JSDoc comprise, que reçoit sa fonction `apply` ou son constructeur de service. Les types référencés sont reproduits à côté lorsqu’ils appartiennent au package, ou reliés à leur source dans les autres cas. La déclaration correspond au type de configuration complet du plugin : un champ que le schéma d’exécution exclut volontairement appartient uniquement au fonctionnement interne, comme l’indique sa propre JSDoc, et ne peut pas être défini depuis `cordis.yml`. Cette référence couvre l’axe du **déploiement**. Le câblage utilisé par un auteur de plugin figure dans la région de l’API Cordis générée de chaque [page de sous-système](subsystems/core.md), les schémas d’outils exposés au modèle dans le [catalogue des outils](tool-catalog.md), et les types cités par ces déclarations dans [subsystems/](subsystems/core.md).

Ce fichier est GÉNÉRÉ depuis les sources par `scripts/gen-config-catalog.ts`, puis sa fraîcheur est vérifiée par `pnpm run verify-config-catalog` dans `doc-sync`; ne le modifiez pas à la main. Les déclarations utilisent une clôture `ts config-catalog`, ignorée par le contrôle de types de la documentation puisqu’une déclaration isolée qui référence des imports ne peut pas être compilée seule. Le générateur compare également le schéma Schemastery utilisé à l’exécution avec la déclaration reproduite : chaque clé validée par le schéma, y compris les clés imbriquées, doit pouvoir être retrouvée sur le type de configuration déclaré. La reproduction ne peut donc masquer aucun champ accepté par le chargeur.

Une ligne `Requires:` énumère les clés de service déclarées dans les `inject` du plugin : son arbre `cordis.yml` doit aussi charger les fournisseurs de ces services. Le catalogue couvre la couche du harness (`packages/`). Les plugins Cordis incorporés qu’un arbre de configuration peut également charger — `hmr`, le journaliseur de console, etc. — proviennent d’une révision amont figée conformément à la [politique d’intégration](../vendor/README.md) et ne sont pas répertoriés ici.

<a id="lasmex-acp"></a>

## `lasmex-acp`

Services requis : `agents`
...
Dépend de : `Stream` (`@agentclientprotocol/sdk`)
...
Dépend de : [`agentCore`](../packages/examples/agent-spine-demo/src/index.ts) · [`JsonlCompression`](../packages/session/session-persistence-jsonl/src/index.ts) · [`ToolsConfig`](#lasmex-tools)
...
Services requis : `agents` · `sessions` · `llm` · `tools` · `systemPrompt`
...
Dépend de : [`AgentOptions`](subsystems/core.md) · [`SessionId`](subsystems/core.md)
...
Dépend de : [`ToolPresentationMode`](subsystems/tools.md)
...
Services requis : `subprocess` · `sandbox` · `sandboxPolicy`
...
Dépend de : [`LocalConfig`](#lasmex-bash-local)
...
Source: [`packages/client/connection/src/index.ts:49`](../packages/client/connection/src/index.ts)

<a id="lasmex-client-hmr"></a>

## `lasmex-client-hmr`

Services requis : `clientModules` · `webServer`
...
Services requis : `llm` · `tokenMeter` · `sessions`
...
Dépend de : [`LocalConfig`](#lasmex-fs-local)
...
Services requis : `agentDefaultModel` · `agents` · `sessions`
...
Services requis : `agentDefaultModel` · `agents` · `attachments` · `directoryPicker` · `llm` · `sessions` · `subagents` · `sessionQuery` · `tools` · `userQuestions` · `workspaceRegistry`
...
Dépend de : [`RetryPolicyConfig`](../packages/llm/llm/src/index.ts)
...
Dépend de : `Api` (`@earendil-works/pi-ai`) · `CacheRetention` (`@earendil-works/pi-ai`) · `Model` (`@earendil-works/pi-ai`) · `ModelThinkingLevel` (`@earendil-works/pi-ai`) · `OpenAICompletionsCompat` (`@earendil-works/pi-ai`) · [`RetryPolicyConfig`](../packages/llm/llm/src/index.ts) · `ThinkingBudgets` (`@earendil-works/pi-ai`) · `Transport` (`@earendil-works/pi-ai`)
...
Dépend de : [`ModelModality`](../packages/llm/llm/src/index.ts) · [`RetryPolicyConfig`](../packages/llm/llm/src/index.ts)
...
Services requis : `fs` · `lsp` · `subprocess`
...
Dépend de : [`MemoryLimits`](subsystems/memory.md)

Source: [`packages/memory/memory-storage-domain/src/index.ts:34`](../packages/memory/memory-storage-domain/src/index.ts)

<a id="lasmex-message-feedback"></a>

## `lasmex-message-feedback`

Services requis : `storageDomain` · `sessionPersistence` · `sessions`
...
Services requis : `shell` · `approval` · `sessions`
...
Dépend de : [`ApprovalPolicy`](subsystems/approval.md) · [`SandboxMode`](subsystems/sandbox.md)
...
Services requis : `tools` · `systemPrompt`
...
Dépend de : [`LocalConfig`](#lasmex-pwsh-local)
...
Dépend de : [`SandboxMode`](subsystems/sandbox.md)
...
Dépend de : `Readable` (`node:stream`) · `Writable` (`node:stream`)
...
Services requis : `sessionProjections`
...
Source: [`packages/session/session-mission/src/index.ts:20`](../packages/session/session-mission/src/index.ts)

<a id="lasmex-session-persistence-jsonl"></a>

## `lasmex-session-persistence-jsonl`

Services requis : `sessions`
...
Services requis : `storageDomain` · `sessionProjections` · `sessionPersistence` · `sessions`
...
Dépend de : [`SessionQueryConfig`](../packages/session-query/session-query/src/index.ts)
...
Dépend de : `BatchLogRecordProcessorOptions` (`@opentelemetry/sdk-logs`) · `OTLPExporterNodeConfigBase` (`@opentelemetry/otlp-exporter-base`)
...
Services requis : `sessionTitle` · `llm` · `sessions`
...
Dépend de : [`SessionTitleLlmConfig`](../packages/session/session-title-llm/src/index.ts)
...
Services requis : `subagents` · `subprocess`
...
Source: [`packages/subagent/subagent-lasmex-sdk/src/index.ts:29`](../packages/subagent/subagent-lasmex-sdk/src/index.ts)
...
Services requis : `terminals` · `sandboxPolicy` · `subprocess`
...
Services requis : `tools` · `shell` · `systemPrompt` · `shellEnv`
...
Services requis : `tools` · `terminals`
...
Services requis : `tools` · `fs` · `systemPrompt`
...
Services requis : `tools` · `systemPrompt` · `subprocess`
...
Services requis : `agents` · `goals` · `tools` · `systemPrompt`
...
Services requis : `tools` · `jobs` · `systemPrompt`
...
Services requis : `tools` · `lsp` · `systemPrompt`
...
Services requis : `tools` · `memory` · `systemPrompt`
...
Services requis : `tools` · `workflowEngine` · `subagents` · `systemPrompt`
...
Services requis : `tools` · `systemPrompt` · `sessionQuery`
...
Services requis : `agents` · `tools` · `skills`
...
Services requis : `tools` · `subagents` · `systemPrompt`
...
Dépend de : [`AgentOptions`](subsystems/core.md)
...
Services requis : `subagents` · `tools` · `systemPrompt`
...
Dépend de : [`SubagentReportDelivery`](subsystems/subagent.md)
...
Services requis : `terminals` · `tools` · `systemPrompt`
...
Services requis : `tools` · `web` · `systemPrompt`
...
Services requis : `tools` · `workflowEngine` · `systemPrompt`
...
## Plugins chargeables sans configuration

Ces plugins se chargent depuis une entrée de `cordis.yml` dépourvue de bloc `config:` ; ils ne déclarent aucune API de configuration.

- `lasmex-agent` ([`packages/core/agent/src/index.ts`](../packages/core/agent/src/index.ts))
- `lasmex-api-gateway` — requiert `typert` ([`packages/api/gateway/src/index.ts`](../packages/api/gateway/src/index.ts))
...
- `lasmex-client-modules` — requiert `loader` ([`packages/client/modules/src/index.ts`](../packages/client/modules/src/index.ts))
...
- `lasmex-client-ui-deliverables` — requiert `systemPrompt` ([`packages/client/ui-deliverables/src/index.ts`](../packages/client/ui-deliverables/src/index.ts))
...
- `lasmex-command-compact` — requiert `commands` · `compaction` ([`packages/compaction/command-compact/src/index.ts`](../packages/compaction/command-compact/src/index.ts))
- `lasmex-command-feedback` — requiert `commands` ([`packages/feedback/command-feedback/src/index.ts`](../packages/feedback/command-feedback/src/index.ts))
- `lasmex-command-goal` — requiert `commands` · `goals` ([`packages/goal/command-goal/src/index.ts`](../packages/goal/command-goal/src/index.ts))
...
- `lasmex-fs-e2b` — requiert `e2b` ([`packages/e2b/fs-e2b/src/index.ts`](../packages/e2b/fs-e2b/src/index.ts))
...
- `lasmex-goal-round-driver` — requiert `agents` · `goals` · `sessions` ([`packages/goal/goal-round-driver/src/index.ts`](../packages/goal/goal-round-driver/src/index.ts))
- `lasmex-host-directory-picker-auto` — requiert `webServer` · `loader` ([`packages/host/directory-picker-auto/src/index.ts`](../packages/host/directory-picker-auto/src/index.ts))
...
- `lasmex-host-plugin-inventory` — requiert `loader` ([`packages/host/plugin-inventory/src/index.ts`](../packages/host/plugin-inventory/src/index.ts))
...
- `lasmex-schedule` — requiert `agents` · `sessions` · `tools` · `sessionPersistence` ([`packages/schedule/schedule/src/index.ts`](../packages/schedule/schedule/src/index.ts))
- `lasmex-session` ([`packages/core/session/src/index.ts`](../packages/core/session/src/index.ts))
- `lasmex-session-checkpoint-policy` — requiert `llm` · `sessionPersistence` · `sessions` · `tools` ([`packages/session/session-checkpoint-policy/src/index.ts`](../packages/session/session-checkpoint-policy/src/index.ts))
- `lasmex-session-log-export` — requiert `commands` ([`packages/session-query/session-log-export/src/index.ts`](../packages/session-query/session-log-export/src/index.ts))
...
- `lasmex-session-stats` — requiert `sessionProjections` ([`packages/session/session-stats/src/index.ts`](../packages/session/session-stats/src/index.ts))
- `lasmex-skill-badge` — requiert `skills` ([`packages/skill/skill-badge/src/index.ts`](../packages/skill/skill-badge/src/index.ts))
...
- `lasmex-tool-ask-user` — requiert `tools` · `userQuestions` ([`packages/interaction/tool-ask-user/src/index.ts`](../packages/interaction/tool-ask-user/src/index.ts))
- `lasmex-tool-call-timeout-policy` — requiert `tools` ([`packages/guard/timeout-policy/src/index.ts`](../packages/guard/timeout-policy/src/index.ts))
- `lasmex-tool-cordis` — requiert `tools` · `systemPrompt` · `dynamicCordisRunner` · `cordisInspect` ([`packages/extensions/tool-cordis/src/index.ts`](../packages/extensions/tool-cordis/src/index.ts))
- `lasmex-tool-subagent-control` — requiert `tools` · `subagents` ([`packages/subagent/tool-subagent-control/src/index.ts`](../packages/subagent/tool-subagent-control/src/index.ts))
- `lasmex-user-questions` ([`packages/interaction/user-questions/src/index.ts`](../packages/interaction/user-questions/src/index.ts))
- `lasmex-workspace` — requiert `storageDomain` · `sessionPersistence` ([`packages/workspace/workspace/src/index.ts`](../packages/workspace/workspace/src/index.ts))

## Packages de capacité non chargeables directement

Ces classes de service sont abstraites : un déploiement charge à leur place un package d’implémentation concret ([capacités complètes](../.agents/notes/implemented/architecture/2026-06-13-capability-seams.md)).

- `lasmex-attachment` — service abstrait `AttachmentStore` ([`packages/attachment/attachment/src/index.ts`](../packages/attachment/attachment/src/index.ts))
- `lasmex-code-runtime` — service abstrait `CodeRuntime` ([`packages/code-runtime/code-runtime/src/index.ts`](../packages/code-runtime/code-runtime/src/index.ts))
- `lasmex-compaction` — service abstrait `CompactionEngine` ([`packages/compaction/compaction/src/index.ts`](../packages/compaction/compaction/src/index.ts))
- `lasmex-credentials` — service abstrait `CredentialProvider` ([`packages/credentials/credentials/src/index.ts`](../packages/credentials/credentials/src/index.ts))
- `lasmex-fs` — service abstrait `FileSystem` ([`packages/fs/fs/src/index.ts`](../packages/fs/fs/src/index.ts))
- `lasmex-host-directory-picker` — service abstrait `DirectoryPicker` ([`packages/host/directory-picker/src/index.ts`](../packages/host/directory-picker/src/index.ts))
- `lasmex-jobs` — service abstrait `JobRegistry` ([`packages/jobs/jobs/src/index.ts`](../packages/jobs/jobs/src/index.ts))
- `lasmex-memory` — service abstrait `MemoryService` ([`packages/memory/memory/src/index.ts`](../packages/memory/memory/src/index.ts))
- `lasmex-sandbox` — service abstrait `SandboxProvider` ([`packages/sandbox/sandbox/src/index.ts`](../packages/sandbox/sandbox/src/index.ts))
- `lasmex-session-persistence` — service abstrait `SessionPersistence` ([`packages/session/session-persistence/src/index.ts`](../packages/session/session-persistence/src/index.ts))
- `lasmex-session-query` — service abstrait `SessionQueryEngine` ([`packages/session-query/session-query/src/index.ts`](../packages/session-query/session-query/src/index.ts))
- `lasmex-settings` — service abstrait `SettingsProvider` ([`packages/settings/settings/src/index.ts`](../packages/settings/settings/src/index.ts))
- `lasmex-shell` — service abstrait `ShellExecutor` ([`packages/shell/shell/src/index.ts`](../packages/shell/shell/src/index.ts))
- `lasmex-spill` — service abstrait `SpillStore` ([`packages/spill/spill/src/index.ts`](../packages/spill/spill/src/index.ts))
- `lasmex-subprocess` — service abstrait `SubprocessRuntime` ([`packages/subprocess/subprocess/src/index.ts`](../packages/subprocess/subprocess/src/index.ts))
- `lasmex-workflow` — service abstrait `WorkflowEngine` ([`packages/workflow/workflow/src/index.ts`](../packages/workflow/workflow/src/index.ts))

## Packages de bibliothèque sans entrée de plugin

Ces packages sont importés comme bibliothèques par d’autres packages ; un fichier `cordis.yml` ne peut pas les charger.
