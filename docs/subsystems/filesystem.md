...
The optional filesystem capability has four parts: [lasmex-fs](../../packages/fs/fs) owns `ctx.fs` and atomic text operations with optional guards, [lasmex-fs-local](../../packages/fs/fs-local) implements local disk, [lasmex-fs-observation-policy](../../packages/fs/fs-observation-policy) records observed presence or absence and adds freshness rules through events rather than a service, and [lasmex-tool-fs](../../packages/fs/tool-fs) directly executes model-facing read/write/edit calls and renders windows. It is outside the agent-loop spine; alternate backends do not change policy or tool schemas.

`lasmex-fs-observation-policy` is optional. Without it, the `FileSystem` Service Definition, a provider, and the `lasmex-tool-fs` Consumer form the complete, unconstrained filesystem seam: `write` unconditionally creates or overwrites, and `edit` unconditionally replaces literal text. The policy plugin changes these operations by deciding the `fs/*` waterfalls. Removing it does not break the tool because the tool calls `ctx.fs` and dispatches events; it does not call policy methods. A deployment that loads `lasmex-tool-fs` is expected to also load `lasmex-fs-observation-policy` so the default behavior is read-before-write/edit.
...
`lasmex-fs` owns three events the tool dispatches and the policy plugin listens for, so the emitter (`lasmex-tool-fs`) and the listener (`lasmex-fs-observation-policy`) share a vocabulary without the emitter depending on the policy plugin. They carry only `lasmex-fs` vocabulary plus an opaque `object` actor — no model-facing concepts and no agent/session owner structure.
...
The policy plugin needs just enough execution context to derive the observed-state owner by narrowing the opaque `object` actor the `fs/*` events carry. `ToolExecution` has the required fields, so `lasmex-tool-fs` passes its execution object through as the actor without making `lasmex-fs-observation-policy` import the tool, agent, or session packages.
