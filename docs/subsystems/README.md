...
| [subprocess.md](subprocess.md) | the subprocess seam: fully-explicit `SubprocessSpawnSpec`, offset-based output readers, unclassified `SubprocessOutcome`, and the managed `LASMEX_*` environment vocabulary |
...
| [memory.md](memory.md) | explicit project-scoped long-term memory: branded ids, bounded durable records, mutation admission, and replayable pinned context |
...
| [client-modules.md](client-modules.md) | the web plugin table: `lasmex.client` declarations, `WebBootGraph` wire composition, the bundle route and index tap |
