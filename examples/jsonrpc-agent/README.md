...
| `DEEPSEEK_BASE_URL` | Host endpoint used by `lasmex-llm-deepseek` |
| `LASMEX_CWD` | Agent workspace for bash and filesystem tools |
| `LASMEX_CONTEXT_WINDOW` | Context capacity recorded for the `LASMEX_MODEL` catalog entry in the minimal variant |
| `LASMEX_MAX_TOKENS_AS_SUCCESS` | `true` (default) accepts token-limited results; `false` reports them as errors |
| `LASMEX_MODEL` | Default model used by `minimal.py`; `--model` takes precedence |
| `LASMEX_SESSION_ROOT` | JSONL session directory |
| `LASMEX_SYSTEM_PROMPT` | Deployment-provided coding persona |

Pass the config path through the Python SDK's `cordis` option or `LASMEX_CORDIS_CONFIG`. The bundled executable already carries every plugin named by this file; the target machine does not need Node.js.

## Minimal variant

[`minimal.cordis.yml`](minimal.cordis.yml) is the complete standalone counterpart of the Web `minimal` preset. `LASMEX_SYSTEM_PROMPT` selects its system prompt, with `You are a helpful software engineer assistant.` as the fallback. It suppresses every system-prompt runtime-context contribution for fresh sessions and mounts no context-compaction plugin. Its model-facing tools are exactly:
