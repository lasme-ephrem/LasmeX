...
describe('jsonrpc-agent keyless smoke', () => {
  it.each([
    ...
    const root = await mkdtemp(join(tmpdir(), 'lasmex-jsonrpc-agent-smoke-'))
    ...
    const child = execa(process.execPath, [
      ...
      env: {
        ...
        LASMEX_CWD: root,
        LASMEX_SESSION_ROOT: join(root, '.sessions'),
        ...(envValue === undefined ? {} : { LASMEX_MAX_TOKENS_AS_SUCCESS: envValue }),
      },
      timeout: 35_000,
      killSignal: 'SIGKILL',
      reject: false,
    ...
    try {
      ...
      expect(initialized).toMatchObject({
        jsonrpc: '2.0',
        id: 1,
        result: { serverInfo: { name: 'lasmex-sdk-runtime' } },
      })
  ...
  it('rejects an invalid max-token success env value', async () => {
    const { exitCode, stdout, stderr } = await execa(process.execPath, [
      ...
      env: {
        DEEPSEEK_API_KEY: 'keyless-smoke-no-call',
        LASMEX_MAX_TOKENS_AS_SUCCESS: 'sometimes',
      },
      stdin: 'ignore',
      timeout: 25_000,
      killSignal: 'SIGKILL',
    ...
    expect(stdout).toBe('')
    expect(stderr).toContain("échec du chargement de l'arbre de plugins")
    expect(stderr).toContain('failed to apply loader entry sdk-jsonrpc-server (lasmex-sdk-jsonrpc-server)')
