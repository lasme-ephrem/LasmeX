...
class CliMockAdapter extends LlmAdapter {
  ...
  async * stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    if (toolResult === undefined) {
      ...
      yield { type: 'tool-call-delta', index: 0, id: CallId('cli-memory-list-call'), name: 'memory_list', argumentsDelta: args }
      yield { type: 'block-end', index: 0, block: { type: 'tool-call', id: CallId('cli-memory-list-call'), name: 'memory_list', arguments: args } }
    ...
    const toolText = toolResult.content
      ...
      .join('')
    const reply = `CLI memory round trip complete: ${toolText.trim()}`
