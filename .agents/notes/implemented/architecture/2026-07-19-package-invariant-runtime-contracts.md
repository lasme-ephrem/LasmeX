...
The central `lasmex-invariants` service owns only configuration, registration uniqueness, child-fiber lifecycle, rollback, disposal, and package-attributed failure. It exposes no generic plugin-shape, service-shape, or startup-assertion helpers and imports no product package.
...
|---|---|
| `lasmex-session` | Strict sequence growth, turn/step enclosure, and same-step tool call/result pairing. |
| `lasmex-agent` | Non-repeating agent status and terminal disposal transitions. |
| `lasmex-scope` | Scoped-event carrier presence and routed-subject consistency. |
| `lasmex-agent-loop` | Explicitly marked, frozen loop request reconstruction from the session event log. |
| `lasmex-llm` | Stream block grammar, delta type/index matching, single usage, closed blocks, and terminal finish. |
| `lasmex-llm-retry` | Durable retry records identify the open turn's latest closed step, remain unique per step, increase monotonically, and stay within retry and non-negative timer bounds. |
| `lasmex-tools` | Monotonic pre/execute/post stages and immutable final execution/result snapshots. |
| `lasmex-system-prompt` | Authoritative assembly section, tool, and variable data constraints. |
| `lasmex-compaction` | Compaction start/summary/end pairing, range endpoints, token counts, and successful-summary presence. |
| `lasmex-hook-protocol` | Hook invocation/result correlation, dialect, identity, and duration constraints. |
| `lasmex-sandbox-policy` | Durable `sandbox/mode` events use the closed sandbox-mode vocabulary. |
| `lasmex-fs` | Filesystem decision/observation events carry usable target and version identities. |
| `lasmex-goal` | Durable goal snapshots preserve source attribution, rendered content, revisions, lifecycle and timestamp relationships, and sequential admitted rounds. |
| `lasmex-goal-round-driver` | Goal-sourced continuation messages match the prompt reconstructed from the preceding durable goal state. |
| `lasmex-subagent` | Provider add/remove and child start/end events preserve identity and pairing. |
| `lasmex-permission-presets` | Durable permission decisions name a preset in the active permission table. |
| `lasmex-user-approval` | Approval asked/decided records pair by call and use valid outcomes and policies. |
| `lasmex-workflow` | Workflow and child-agent start/end events preserve run metadata, identity, outcome, count, and error relations. |
| `lasmex-jobs` | Current and terminal task snapshots preserve id/kind, owner, status, and timestamp relationships. |
| `lasmex-tool-todo` | Durable whole-list snapshots use unique trimmed items and closed statuses. |
| `lasmex-time-context` | Plugin-attributed clock readings agree with the session's open turn, next pre-step position, and elapsed baseline; rendered time parses and does not postdate its event. |
