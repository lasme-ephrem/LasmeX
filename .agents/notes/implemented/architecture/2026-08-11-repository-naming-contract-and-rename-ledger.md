...
The repository also used `SDK` in two meanings. The supported Python and TypeScript clients use the JSON-RPC SDK protocol. The project as a whole is LasmeX, not an SDK project. The removed SDK project toolchain made the broad meaning obsolete, but prose and names preserved parts of it.
...
`SDK` means the JSON-RPC-based client/server protocol used by the supported Python and TypeScript SDKs. The repository keeps `lasmex-sdk-client` and `lasmex-sdk-protocol`; the wire identity is `lasmex-sdk-runtime`, and the JSON-RPC server belongs to the same family. LasmeX itself is not an SDK, and the removed project generator, launcher, helper, and launcher telemetry packages stay absent. The [LasmeX Python SDK public identity decision](2026-08-14-lasmex-python-sdk-public-identity.md) partially supersedes this decision only for the Python names, executable lineage, TypeScript high-level API, and server identity; the npm import specifiers remain here until their complete package-scope family moves.
...
Do not remove an intentional product qualifier to avoid repetition. The `lasmex-subagent-lasmex-sdk` npm name identifies the LasmeX SDK provider while retaining the inherited package-scope prefix and avoids confusion with another SDK. Its private class remains `SdkSubagentProvider` because the class also needs to say what it provides.
...
|---|---|---|
| `lasmex-jsonrpc` | `lasmex-sdk-jsonrpc-server` | It is the server half of the SDK protocol. `jsonrpc` alone names an encoding; `sdk-jsonrpc-server` gives the family, mechanism, and role. |
| `HarnessSdkServer` | `HarnessSdkJsonRpcServer` | The class is one JSON-RPC server implementation, not every possible SDK server. |

Keep `lasmex-sdk-client` and `lasmex-sdk-protocol`; use `lasmex-sdk-runtime` for `serverInfo.name`. Exclude `@deepseek-ai/create-sdk`, `lasmex-scripts`, `lasmex-helper`, and `lasmex-telemetry`; the separate removal decision deletes them and their support graph.
...
| `lasmex-bash`, `ctx.bash` | `lasmex-shell`, `ctx.shell` | PowerShell already implements this seam. The capability is shell execution, not Bash. |
...
| `lasmex-bash-env`, `ctx.bashEnv`, `BashEnvRegistry` | `lasmex-shell-env`, `ctx.shellEnv`, `ShellEnvRegistry` | The environment registry is shared by Bash and PowerShell tools. |
...
| `lasmex-pty`, `ctx.pty`, `PtyService` | `lasmex-terminal`, `ctx.terminals`, `TerminalSessionService` | Callers manage multiple named terminal sessions. They do not allocate raw PTYs through this service. |
...
| `lasmex-pty-local`, `LocalPtyBackend` | `lasmex-terminal-bash`, `BashTerminalBackend` | The provider depends on Bash prompt and shell behavior. `local` hides the actual dialect. |
| `lasmex-tool-pty` | `lasmex-tool-terminal` | The model-facing tools are already `terminal_*`; the package should use the same product noun. |
...
| Former | Current | Reason |
|---|---|---|
| `lasmex-lsp-local` | `lasmex-lsp-stdio` | The provider speaks LSP over stdio through replaceable filesystem and subprocess services. It is not necessarily local. |
...
| `lasmex-tasks`, `ctx.tasks`, `TaskService` | `lasmex-jobs`, `ctx.jobs`, `JobRegistry` | The service registers, owns, observes, waits for, and cancels multiple background jobs. It is a registry, not a general task service. |
...
| `lasmex-tasks-local`, `LocalTaskService` | `lasmex-jobs-local`, `LocalJobRegistry` | This is the process-local provider of the job registry. Here `local` is meaningful because the jobs and callbacks live in one process. |
| `lasmex-tool-tasks` | `lasmex-tool-jobs` | The consumer controls the job registry and should use the same domain noun. |
...
| `lasmex-client-ui-task`, `client/ui-task/` | `lasmex-client-ui-jobs`, `client/ui-jobs/` | The client package presents the background-job collection. It is not one user task. |
...
| Former | Current | Reason |
|---|---|---|
| `lasmex-client-ui-slash`, `ui-slash/` | `lasmex-client-ui-input-trigger`, `ui-input-trigger/` | The client handles `/`, `@`, keyboard arbitration, candidate menus, and programmatic launch. It is not only slash commands. |
...
| `lasmex-agent-tool-mode`, plugin `tool-mode` | `lasmex-agent-tool-presentation`, plugin `tool-presentation` | The plugin changes how tools are presented to the model. It does not change execution behavior. Keep local `Config.mode` and `ToolPresentationMode`. |
...
| `lasmex-permission`, `ctx.permission`, `PermissionService` | `lasmex-permission-presets`, `ctx.permissionPresets`, `PermissionPresetService` | The service selects and persists presets. Sandbox and approval services enforce the result. |
| `lasmex-client-ui-permission` | `lasmex-client-ui-permission-presets` | The UI edits and selects permission presets. |
...
| `lasmex-user-interaction`, `user-interaction/` | `lasmex-user-questions`, `user-questions/` | The seam supports question batches and answers only. Approval, commands, and directory picking are separate interaction seams. |
| `ctx.userInteraction`, `UserInteractionService`, `UserInteractionProvider`, `UserInteractionError` | `ctx.userQuestions`, `UserQuestionService`, `UserQuestionProvider`, `UserQuestionError` | These names state the one supported interaction form. Keep `AskUserQuestion*`, the `ask_user_question` tool, and `lasmex-tool-ask-user`. |
...
| Former | Current | Reason |
|---|---|---|
| `packages/typert/type-meta/`, `lasmex-type-meta` | `typert/protocol/`, `lasmex-typert-protocol` | The package owns the Typert Remote protocol, decorators, bindings, codecs, lookups, and context contracts. It is not generic type metadata. |
...
Keep `lasmex-tools` and `ctx.tools`. Keep `lasmex-api-gateway`, its `gateway/` folder, `ctx.typertGateway`, and `TypertGatewayService`; that service is a real API gateway. Its internal `TypeRT*` identifiers still follow the `Typert*` spelling rule.

### Workspace instructions, telemetry, identity, and launch environment

| Former | Current | Reason |
|---|---|---|
| Host `ctx.workspace` | Host `ctx.workspaceRegistry` | `WorkspaceRegistry` owns multiple workspaces, but Client `ctx.workspaces` already has an incompatible type. Both declarations merge into the same Cordis `Context` interface at compile time even though their runtime contexts are separate. The role suffix states the host service and avoids that collision. Keep `lasmex-workspace`, `WorkspaceRegistry`, `Workspace`, and `workspace.*` wire names. |
| `lasmex-workspace-context`, `context/workspace-context/` | `lasmex-agent-instructions`, `context/agent-instructions/` | The package loads hierarchical `AGENTS.md` and `CLAUDE.md` files for the agent. It is not general workspace context. |
...
| `TelemetryOtel`, `TelemetryMode`, plugin `telemetry-otel` | `OpenTelemetrySessionBackend`, `SessionTelemetryMode`, plugin `session-telemetry-otel` | The provider name states both the OpenTelemetry mechanism and session scope. Keep the package names `lasmex-session-telemetry` and `lasmex-session-telemetry-otel`. |
...
| `session/user-id/`, `lasmex-user-id` | `identity/anonymous-user-id/`, `lasmex-anonymous-user-id` | The value is a random correlation id shared by telemetry, feedback, and DeepSeek requests. It is neither a Session concern nor an authenticated user identity. |
...
| `util/environment/`, `lasmex-environment` | `util/launch-environment/`, `lasmex-launch-environment` | The package captures one immutable layered snapshot at launch. It is not a general environment API. |
| Public `Environment*`, `createEnvironmentSnapshot`, `environmentOf`, `DSH_ENVIRONMENT_KEY` | `LaunchEnvironment*`, `createLaunchEnvironmentSnapshot`, `launchEnvironmentOf`, `LASMEX_LAUNCH_ENVIRONMENT_KEY` | The names state the snapshot's lifetime and purpose. |
...
| Former | Current | Reason |
|---|---|---|
| `lasmex-tool-schedule`, `schedule/tool-schedule/`, plugin `tool-schedule` | `lasmex-schedule`, `schedule/schedule/`, plugin `schedule` | The package owns the durable Schedule domain, persistence barriers, management tools, timers, follow-ups, and runtime lifecycle. `tool-` describes only one part. |
...
| `WorkflowService`, `ctx.workflows` | `WorkflowEngine`, `ctx.workflowEngine` | One engine parses and executes workflow programs. The plural key wrongly suggests a registry. Keep `lasmex-workflow` and workflow events and tools. |
| `lasmex-workflow-workerthread`, `WorkerWorkflowEngine` | `lasmex-workflow-worker-thread`, `WorkerThreadWorkflowEngine` | `worker thread` is the precise Node mechanism and the repository spelling uses the full words. |
| `lasmex-goal-session`, `goal/goal-session/` | `lasmex-goal-round-driver`, `goal/goal-round-driver/` | The plugin drives same-session Goal Rounds. It neither stores goals nor defines sessions. Keep `GoalService`, goal source, events, and contracts. |
| `packages/compact/` | `packages/compaction/` | The group is a noun-domain family. `compact` remains the user command verb. |
| `lasmex-compact`, `ctx.compact`, `CompactService` | `lasmex-compaction`, `ctx.compaction`, `CompactionEngine` | The object runs the compaction algorithm and lifecycle. It is an engine, not a generic service. |
...
| `lasmex-compact-basic`, `BasicCompactService`, public `BasicCompact*` | `lasmex-compaction-basic`, `BasicCompactionEngine`, corresponding `BasicCompaction*` | `basic` is plain but honest. `compaction-llm` adds no information because LLM use is already part of the current implementation family. |
| `lasmex-compact-tool-result-prune`, `ToolResultPruneService`, `ctx.toolResultPrune` | `lasmex-compaction-tool-result-pruner`, `ToolResultPruner`, `ctx.toolResultPruner` | The plugin is an actor that prunes tool results. The noun `pruner` names that role. |
...
| `lasmex-settings-local`, `SettingsLocal` | `lasmex-settings-file`, `FileSettingsProvider` | The implementation is file-backed through the filesystem seam. `file` states the mechanism; `local` does not. |
...
| Former | Current | Reason |
|---|---|---|
| `HttpServerService`, `ctx.httpServer` | `WebServer`, `ctx.webServer` | The server owns HTTP routes and WebSocket upgrade routes. `Web` leaves room for both; `Http` is too narrow here. Keep `packages/host/webserver`, `lasmex-host-webserver`, `WebRoute`, and `WebUpgradeRoute`. |
...
| `lasmex-session-title-first-message-llm`, cadence `first-message` | `lasmex-session-title-first-prompt-llm`, cadence `first-prompt` | The trigger is the first user prompt, not any message in the session log. |
| `lasmex-session-title-all-messages-llm`, cadence `all-user-messages` | `lasmex-session-title-all-prompts-llm`, cadence `all-prompts` | The backend refreshes from user prompts. `all messages` wrongly includes assistant and tool events. |
| `lasmex-code-runtime-worker`, `WorkerCodeRuntime` | `lasmex-code-runtime-worker-thread`, `WorkerThreadCodeRuntime` | The implementation uses a Node worker thread. `worker` alone is too broad. |
...
| Former | Current | Reason |
|---|---|---|
| `lasmex-fs-policy` | `lasmex-fs-observation-policy` | The package defines which filesystem observations authorize later effects. It is not the complete filesystem or sandbox policy. |
...
| `lasmex-skill-local`, `LocalSkillProvider`, provider id `local` | `lasmex-skill-filesystem`, `FileSystemSkillProvider`, provider id `filesystem` | The provider discovers skill files through `ctx.fs`, which can be local or remote. The mechanism is filesystem access, not locality. |
...
| `lasmex-subagent-spawn`, `SpawnProvider` | `lasmex-subagent-spawn-in-process`, `SpawnInProcessProvider` | This provider starts a child agent in the current process. The configured provider id remains `spawn`. |
| `lasmex-subagent-fork`, `ForkProvider` | `lasmex-subagent-fork-in-process`, `ForkInProcessProvider` | This provider forks an agent in the current process. The configured provider id remains `fork`. |
| `lasmex-subagent-inprocess`, `subagent-inprocess/` | `lasmex-subagent-in-process-driver`, `subagent-in-process-driver/` | The package contains common in-process driving logic, not a third provider. |
| Private `SdkProvider` in `lasmex-subagent-lasmex-sdk` | `SdkSubagentProvider` | The product-qualified package is intentional, and the class must say that it provides subagents through the SDK. |
...
| `lasmex-web-fetch-local`, `LocalFetchProvider`, `LocalFetchLimits`, provider id `local-http` | `lasmex-web-fetch-http`, `HttpFetchProvider`, `HttpFetchLimits`, provider id `http` | This provider performs direct HTTP fetches. `local` says where code happens to run, not which mechanism it provides. |

Keep `lasmex-subagent-lasmex-sdk`, its provider id `lasmex-sdk`, external ACP, Codex, and Claude Code provider families, the subagent tool package names, the main filesystem package and backends, filesystem tools and events, and the skill badge and tool packages.

### Hooks, guards, plan mode, extensions, and diagnostics

| Former | Current | Reason |
|---|---|---|
| `lasmex-hooks-claude`, `ClaudeHookConfig`, `parseClaudeConfig`, dialect `claude` | `lasmex-hooks-claude-code`, `ClaudeCodeHookConfig`, `parseClaudeCodeConfig`, dialect `claude-code` | The hook bridge targets Claude Code, not every Anthropic or Claude product. |
| `lasmex-repeat-tool-guard`, plugin/source `repeat-tool-guard` | `lasmex-repeat-tool-reminder`, plugin/source `repeat-tool-reminder` | The plugin adds a model reminder. It does not block or enforce a guard decision. |
| `lasmex-timeout-policy` | `lasmex-tool-call-timeout-policy` | The full `tool-call` qualifier names what the policy limits without calling the plugin a model-facing tool. Keep its `guard/timeout-policy/` directory and plugin id `timeout-policy`; the `packages/*/tool-*` catalog convention still applies only to packages that register tools. |
...
| `InvariantService` | `InvariantRegistry` | The object owns registered invariant checks. Keep `lasmex-invariants` and `ctx.invariants`. |
...
| Former | Current | Reason |
|---|---|---|
| `util/paths/`, `lasmex-paths` | `util/home-paths/`, `lasmex-home-paths` | The helpers resolve paths under the Harness home. They are not a general path library. Keep the individual function names when they already state the returned path. |
| `util/retention/`, `lasmex-retention` | `util/output-retention/`, `lasmex-output-retention` | The policy retains command and tool output. It is not a general data-retention framework. |
| `E2BSandboxService` | `E2BRuntime` | The class creates, reuses, and disposes the E2B execution environment used by filesystem and subprocess adapters. It is broader than one sandbox handle and narrower than a generic owner. Keep `lasmex-e2b`, `ctx.e2b`, and the `e2b/` group. |
| `lasmex-frontend-static` | `lasmex-host-frontend-static` | The package is the Host plugin that serves the frontend assets. The prefix distinguishes it from frontend application code. |
...
| `lasmex-jsonrpc-demo` | `lasmex-sdk-jsonrpc-demo` | The example demonstrates the runtime SDK over JSON-RPC. It belongs to the one SDK meaning. |
| `lasmex-frontend` | `lasmex-web-frontend` | The application is the web frontend. Keep its physical `apps/web/` folder. |

Keep atomic-write, brand, native-command, timeout utility, directory-picker, `lasmex-base`, `lasmex-web-app`, app boot, CLI names, and the `headless` package, bundle, and example identity. `headless` is the intended product essence and may later support more than one-shot execution.
...
| `lasmex-client-ui-model` | `lasmex-client-ui-model-selection` | The package controls the model selection for a session. The singular `model` name is too broad. |
...
| `lasmex-client-ui-models` | `lasmex-client-ui-settings-models` | This package owns the Models settings panel. Keep `ModelsSettingsStore`; it holds one settings view model with data operations and subscriptions and is a real store. |
| `lasmex-client-ui-plugin-config`, `client/ui-plugin-config/` | `lasmex-client-ui-settings-plugins`, `client/ui-settings-plugins/` | This package owns the Plugins settings section, not a general plugin-configuration system. The target joins the `ui-settings-*` family and uses the section's plural product name. |
...
| `lasmex-client-ui-plugins`, `client/ui-plugins/`, Loader id `ui-plugins`, `client-ui-plugins-invariant` | `lasmex-client-ui-settings-plugin-inventory`, `client/ui-settings-plugin-inventory/`, Loader id `ui-settings-plugin-inventory`, `client-ui-settings-plugin-inventory-invariant` | This later package owns the read-only Plugin Inventory tab in the Plugins settings section. `ui-plugins` is too broad and does not distinguish the inventory from editable plugin settings. |
...
| `lasmex-client-ui-feedback`, `client/ui-feedback/`, Loader id `ui-feedback`, `client-ui-feedback-invariant` | `lasmex-client-ui-message-feedback`, `client/ui-message-feedback/`, Loader id `ui-message-feedback`, `client-ui-message-feedback-invariant` | This package presents ratings and notes for assistant messages through the `messageFeedback` Remote. The old name also appears to cover command feedback and any later feedback UI. It does not. |
...
| `lasmex-client-ui-question` | `lasmex-client-ui-user-questions` | The UI presents the user-question seam, not an arbitrary question domain. |
| `lasmex-client-ui-command`, `ui-command/` | `lasmex-client-ui-commands`, `ui-commands/` | The package presents and runs a collection of commands. |
| `lasmex-client-ui-directory-picker`, `client/ui-directory-picker/`, Loader id `ui-directory-picker`, `client-ui-directory-picker-invariant` | `lasmex-client-ui-directory-picker-browse`, `client/ui-directory-picker-browse/`, Loader id `ui-directory-picker-browse`, `client-ui-directory-picker-browse-invariant` | The Client packages now contain separate `browse` and `native` directory-picker presentations. The unqualified package is the browse implementation, not their shared definition. The target matches the Host backend family and changes no boundary. |
...
- Keep `lasmex-api-gateway`, `ctx.typertGateway`, and `TypertGatewayService`.
- Keep session projection names. A projection is not only a reducer function.
- Keep `lasmex-session-stats`, `sessionStats`, and `SessionStatsProjection`. They accurately name whole-session statistics and the maintained read model that carries them.
...
- Keep `lasmex-subagent-lasmex-sdk` and provider id `lasmex-sdk`; the product qualifier prevents ambiguity.
...
- Keep `lasmex-client-ui-directory-picker-native`; its suffix names the native-chooser presentation beside the renamed `-browse` variant. Keep `SURFACE_PACKAGES`; within the directory-picker auto selector it is the package map for the Client presentation half, contrasted with `BACKEND_PACKAGES`.
- Keep `lasmex-host-plugin-inventory`, `ctx.pluginInventory`, the `pluginInventory/list` Remote, and the `PluginInventory*` payload types. They accurately name the Host-owned read-only inventory; only the adapter class and the overly broad Client presentation names change.
...
**Rename `dsh-compact-basic` to `lasmex-compaction-llm`.** Rejected. `LLM` adds no distinction in the current backend family. `basic` is less ambitious and does not claim an algorithm that does not exist.
...
- The paired package-creation guide contains the role-word contract, `packages/AGENTS.md` links to it, the terminology table records the chosen words and `Typert` spelling, and root project prose calls the product LasmeX rather than LasmeX SDK.
