...
enter 分支的 `PreStepDecision.messages` 是拟议步骤的完整批次。waterfall（瀑布式事件）监听器调用 `next()` 委托时，会保留下游消息，除非有意替换。waterfall 结束后，循环会对已接纳批次进行稳定分组：source 不是 `user` 的消息先进入，随后是 `source.kind === 'user'` 的消息。两个分组都保留生产方顺序和消息标识。工具结果的 `additionalContexts` 保留 FIFO 顺序及每条消息的 source。

仅因工具返回而创建的 continuation 没有新领取的直接消息，无法自然恢复该尾部位置。因此，循环会在工具历史之后追加一条持久插件 notice。该 notice 列出本轮已经调用的所有工具，要求模型不要仅为满足被引用指令而重复这些调用，引用最近直接请求的文本块，并保留精确响应的标点。这种显式重复让能力较弱的模型继续聚焦于尚未完成的响应指令，同时同样明确地标记已完成动作；原始带标识消息在历史中的较早位置保持不变。新接纳的直接消息会取代这条自动 notice。

每个轮次还有一个可配置的正数 `maxStepsPerTurn` 限制。循环会在准入模型请求之前检查该限制；如果另一步会超过上限，则记录 `MAX_STEPS`。即使其他循环卫生策略未能停止重复调用，模型或插件也不能无限延续同一轮次。
...
- 每个已接纳批次都会先记录并发送非 user source 上下文，再发送 FIFO 直接用户消息，直接提示词位于批次尾部。
- 仅由结果触发的 continuation 会在工具历史之后记录已完成工具名称和最近直接文本的引用副本；新的直接消息会抑制该 notice。
- `maxStepsPerTurn` 会在超过所配置上限的第一个请求之前关闭失控轮次。
...
- 连续的 user-role 消息会取代一条烘焙后的提示词消息；提供方适配器会收到循环生成的稳定顺序，即上下文在前、直接用户消息在后。
- 仅由结果触发的工具步骤会增加一条有界插件 notice 和重复的直接文本，同时保留现有请求前缀与完整工具历史。
