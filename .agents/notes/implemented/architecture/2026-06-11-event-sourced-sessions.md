...
- Persistence stays a plugin concern; the in-memory store ships in lasmex-session.
...
- Derivation cost grows with log length — compaction (lasmex-compaction) is the intended mitigation, not log mutation.
