...
| Concern | Owner |
|---|---|
| Validate request hint and clamp default/max | `lasmex-timeout` (`clampTimeout`) — pure arithmetic plus the shared positive-finite request contract |
| Arm one-shot timer, abort on deadline, carry reason, fuse with upstream cancel | `lasmex-timeout` (`deadline`) |
| Arm and rearm only around outstanding iterator demand, including out-of-band activity | `lasmex-timeout` (`idleWatchdog`) |
| Clear the timer | `lasmex-timeout` (`[Symbol.dispose]` on either primitive) |
| Classify the first abort reason after abort | `lasmex-timeout` (`timeoutOf`) |
