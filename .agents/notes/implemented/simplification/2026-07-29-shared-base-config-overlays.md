...
Precedence is list order, last write winning per row: base, then the surface overlay, then either a `--config` overlay or the personal `~/.lasmex/config.yaml`, then the launcher's own flag and profile patches.
...
An overlay or `--config` tree that named `lasmex-tui-demo`, or patched the `tui-agent` row, no longer resolves. Overlays now patch the row that owns each key: the model route on `agent-loop`, the persona on `system-prompt`, presentation on `tui`.
