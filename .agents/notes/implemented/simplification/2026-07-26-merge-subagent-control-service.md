...
- The continuation manager is still Task- and persistence-aware inside the `lasmex-subagent` package, so the package declares optional peer dependencies on those services even though ordinary `start` callers do not need them.
