...
- **One product onboarding path.** The root README owns the recommended package-run path, the source-run alternative, and compact `lasmex plugin --profile` usage. The published user guide starts with tasks inside the running Web UI, then links to distinct tutorials or reference owners for other interfaces, plugin development, and advanced configuration instead of repeating Web startup.
