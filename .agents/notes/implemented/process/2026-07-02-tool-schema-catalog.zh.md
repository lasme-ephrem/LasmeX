...
`packages/*/tool-*` 下已发布的产品工具包，每个都使用默认配置启动，包括 `lasmex-tool-bash`（`bash`）、`lasmex-tool-jobs`（`job_output`、`job_list`、`job_kill`）和 `lasmex-tool-subagent`（`subagent`）。仅供示例使用的工具不在范围内。
