...
The documentation-site lockup inlines the exact-transparent cube logo (`website/public/full.svg`); the app's wide sidebar renders the same embedded logo (`FullLogo`) and the hero and rail mount the embedded cube mark (`CubeMark`), while the favicon ships the embedded cube mark (`apps/web/public/favicon.svg`). A change to the product mark reaches the documentation site only by updating this copy.
