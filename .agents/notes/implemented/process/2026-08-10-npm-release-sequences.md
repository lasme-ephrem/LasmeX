...
`packages/*/*`, `apps/cli`, and `apps/web` form the npm runtime distribution of `lasmex`; the private `apps/desktop` assembly is not an npm release member. `vendor/*` holds nine rescoped Cordis framework packages, each carrying its upstream version; `native/landlock-run/packages/*` holds Linux platform packages with their own workflow. The three differ in version baseline, change rate, and build requirements: LasmeX moves with the product, vendor moves only when upstream is re-synced or a local modification changes, and native needs a musl toolchain and one build per architecture. Forcing them through one pipeline means every product release republishes the framework and the native binaries.

Two hard blockers sat in the way. Publishable workspace manifests set `private: true`, which npm refuses to publish. The subtler one was hand-written `peerDependencies: "^0.0.1"` entries between sibling product packages: `pnpm pack` substitutes the `workspace:` protocol but leaves semver ranges alone, and `^0.0.1` means `>=0.0.1 <0.0.2` — it excludes `0.0.2`, and semver excludes prereleases from a range without a prerelease of its own, so it excluded `0.0.1-rc.1` too. Those entries never failed only because the version never left `0.0.1`.

`scripts/publish-npm-baseline.ts` is a local publication script: it stages the LasmeX and vendored packages, packs and publishes in one process, and needs a human to authenticate and retry on their own machine. It cannot be the basis for CI publication, though its tarball payload validation and installed-artifact probes are verified parts.

## Decision

### Three independent sequences

`packages/`, `vendor/`, and `native/` each have one bump sequence and one publication, sharing no version, no trigger, and no waiting. Releasing LasmeX does not republish vendor; releasing vendor does not republish native.

| Sequence | Members | Version baseline | Tag | Workflow |
|---|---|---|---|---|
| LasmeX | `packages/*/*` + `apps/cli` + `apps/web` (`lasmex` and `lasmex-web-frontend`; private desktop excluded) | one version for the family and the workspace root, `0.0.x` | `lasmex-v<version>` | `release.yml` |
...
All three publish publicly on npmjs.com. LasmeX uses unscoped `lasmex` and `lasmex-*` names; the vendored framework and native packages retain their `@deepseek-ai` scope ([rationale](2026-08-13-public-vendor-and-native-sequences.md)). No publish path passes `--access`: every packed manifest owns its public access level.
...
`scripts/check-workspace-constraints.ts` requires the protocol, so a new package cannot reintroduce a hand-written range; the invariant-companion rule requires `workspace:^` for `lasmex-invariants` for the same reason.
...
The LasmeX family applies the repository's publication payload policy, which rejects sources and declaration maps. The vendored family keeps upstream's payload, because those manifests export `./src/*` and dropping `src` would publish an export map pointing at absent files.
...
A LasmeX verification installs the vendored family's pack output too. The product packages declare the vendored framework as a peer, and one pull request may change both sequences before either reaches the registry, so `release.yml` packs the exact vendored family under test while publishing only its own set.

The verification also packs the Landlock entry, which `lasmex-sandbox-local` declares as a plain dependency, and omits optional dependencies. The platform packages behind those optional entries need a musl toolchain and one build per architecture, so a job on one runner cannot produce them; a consumer that cannot install them must still start, which is what optional means here. The verification therefore reads a directory by its contents rather than a pack order, because a directory can hold tarballs packed only to satisfy a cross-sequence dependency.

### Repository changes this carried

| Item | Content |
|---|---|
| release-set manifests | `private: true` removed and `publishConfig.access: public` set; LasmeX packages omit `repository` until a canonical public repository URL exists |
| release-set boundary | every member of `packages/*/*` and `vendor/*`, plus `apps/cli` and `apps/web`; `apps/desktop` remains private |
...
**Selecting a subset by entry closure.** Crawling `dependencies` from `lasmex` and `lasmex-web-frontend` misses plugins mounted by name from `cordis.yml` rather than imported: `vendor/cordis-plugin-group` and `vendor/cordis-plugin-logger-console` fall outside the dependency closure while being required at runtime. Selecting by code dependency fails as "the consumer installs it and it will not start", and it would need a standing proof that no mounted package was missed. `apps/desktop`, `python/`, the root `examples/`, `docs/`, and `website/` are not members.
...
A pull request runs the full pack for both sequences without credentials and installs the packed LasmeX tarballs into a throwaway consumer, where plain Node drives `lasmex --version`. That probe is deliberately one command: it proves `files` selected a complete payload and that the published ranges resolve, and says nothing about interactive behavior.
...
- **LasmeX packages omit npm source links until the project has a canonical public repository URL.** Inventing a repository field would misdirect consumers and make provenance claims false; adding the real URL later is an explicit metadata update.
...
- **The first publication is one large step.** Nine vendored packages and the whole LasmeX set publish at once, so any payload defect surfaces in a single release, which is why a prerelease version drives the complete path first.
