...
Shipped product tool packages under `packages/*/tool-*`, each booted with its default config, including `lasmex-tool-bash` (`bash`), `lasmex-tool-jobs` (`job_output`, `job_list`, `job_kill`), and `lasmex-tool-subagent` (`subagent`). Example-only tools are excluded.
