...
A blanket statement that the checkout is not the working directory would also be false. `lasmex meta` intentionally makes the source checkout both values.

## Decision

The section identifies the path as the “LasmeX implementation checkout.” It says that the checkout location and current working directory are separate values that may differ, forbids inferring the working directory from the checkout path, directs the model to use `pwd`, and limits the checkout's purpose to inspecting or extending LasmeX itself.
...
**Remove the source path from the prompt.** Rejected because self-referential LasmeX tools still need a reliable checkout location when the launcher starts from an unrelated project.
