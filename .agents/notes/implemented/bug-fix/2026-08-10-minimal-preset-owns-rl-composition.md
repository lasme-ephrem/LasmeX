...
The shipped Web `minimal` preset is the sole Web owner of the RL agent composition. On POSIX it declares an entry-local PTY registry and local backend plus persistent `bash` with the RL environment description and 300-second timeout. On Windows, where the local process inspector cannot support that PTY backend, it omits the PTY group and mounts the shipped one-shot `pwsh` consumer instead. Both variants include `str_replace_editor`, so the preset remains a two-tool composition on every supported desktop platform. Tool presentation remains a deployment choice. The later [bare two-tool runtime decision](../feature/2026-08-11-minimal-profiles-bare-two-tool-runtime.md) supersedes this note's original compaction and filesystem-provider choices: the current preset mounts an entry-local `fs-local` provider and no compaction backend. The editor accepts no `requireAbsolutePath` setting because absolute paths are its unconditional contract.
...
System-prompt and persona package tests prove final complete-section and runtime-context suppression, including waterfall mutation and duplicate rejection. The shipped-preset composition test asserts the exact prompt, platform shell description, absolute editor schema, and two-tool catalog under the default native presentation. The keyless Web replay sends a real request through a `minimal` agent while global identity, Web-orientation text, dynamic policy contexts, and a test section are registered, asserts that no runtime-context snapshot exists, the entry-local filesystem is bare, and compaction is absent, then executes the active platform shell and the editor through an absolute path. The shell-selection test evaluates both platform branches: POSIX retains persistent Bash, while Windows excludes the unsupported PTY group and exposes PowerShell.
...
The Web RL prompt is fixed rather than environment-overridable; the standalone JSON-RPC prompt is deployment-selected. The Web preset and standalone JSON-RPC example retain a two-tool contract for their respective launch paths. On POSIX the model sees persistent `bash` and `str_replace_editor`; shell state is per agent and disappears with that agent. On Windows it sees one-shot `pwsh` and `str_replace_editor`; commands must carry their own cwd and environment state. The Web preset pays for its own PTY instance only on POSIX and its own bare filesystem service on every platform, while other presets pay nothing for them.
