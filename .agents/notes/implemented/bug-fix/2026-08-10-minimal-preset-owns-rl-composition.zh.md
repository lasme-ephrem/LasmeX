...
随附的 Web `minimal` preset 是 RL agent 组合在 Web 中的唯一所有者。在 POSIX 上，它声明 entry 本地的 PTY 注册表与本地后端，并提供带 RL 环境描述且超时为 300 秒的持久 `bash`。在 Windows 上，本地进程检查器无法支持该 PTY 后端，因此 preset 省略 PTY group，改为挂载随产品交付的一次性 `pwsh` Consumer。两种变体都包含 `str_replace_editor`，因此该 preset 在每个受支持的桌面平台上仍然是双工具组合。工具呈现仍由部署选择。后续的[裸双工具运行时决策](../feature/2026-08-11-minimal-profiles-bare-two-tool-runtime.md)取代了本记录最初的压缩与文件系统提供方选择：当前 preset 挂载 entry 本地的 `fs-local` 提供方，不挂载压缩后端。编辑器不接受 `requireAbsolutePath` 设置，因为要求绝对路径是它的无条件约定。
...
系统提示词与 persona 包测试证明了 complete 段最终约束与 runtime-context 抑制，包括 waterfall 修改与重复项拒绝。交付 preset 组合测试在默认原生呈现下断言精确的提示词、平台 shell 描述、要求绝对路径的编辑器 schema 和双工具目录。无密钥 Web 回放通过 `minimal` agent 发送一个真实请求，同时注册全局身份、Web 定位文本、动态策略上下文和一个测试段落；它断言不存在 runtime-context 快照、entry 本地文件系统是裸后端且压缩不存在，随后执行当前平台的 shell，并通过绝对路径执行编辑器。shell 选择测试会同时评估两个平台分支：POSIX 保留持久 Bash，Windows 排除不受支持的 PTY group 并暴露 PowerShell。
...
Web RL 提示词固定不变，不能通过环境覆盖；独立 JSON-RPC 提示词由部署选择。Web preset 与独立 JSON-RPC 示例分别在各自的启动路径保留双工具约定。在 POSIX 上，模型看到持久 `bash` 与 `str_replace_editor`；shell 状态按 agent 隔离，并随该 agent 一并消失。在 Windows 上，模型看到一次性 `pwsh` 与 `str_replace_editor`；每条命令都必须自带 cwd 与环境状态。Web preset 仅在 POSIX 上为自身的 PTY 实例承担开销，并在每个平台为裸文件系统服务实例承担开销；其他 preset 无需承担。
