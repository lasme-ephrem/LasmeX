...
各表面：应用 favicon 与文档站 favicon 发布 `lasmex-icon.svg`（`/favicon.svg`，尺寸 `any` 的 SVG），取代黑色 LX 字标。文档站导航锁式标志内联 `lasmex-full.svg`（44px），取代黑色 LX 字标（`website/public/wordmark.svg` 已删除，`LasmexMark`/`BrandWordmark` 两个 LX 组件随字标一并移除）。应用的 hero 与窄栏挂载描摹方块标记（`CubeMark`）。桌面应用发布描摹图标 `apps/desktop/assets/icon.svg`。应用的宽侧栏渲染完整 logo（`FullLogo`）。`README.md`、`README.zh.md` 与 `LASMEX.md` 以完整 logo 开头。
