...
Storage remains provider-owned. Memorix uses `~/.memorix/data` and Engram uses `~/.engram` by default. The Reference Memory example sets a stable `$HOME/.dsh-mcp-reference-memory.jsonl` path instead of writing into the installed npm package directory. Each provider's own environment variable can override these locations before LasmeX starts.

Project identity remains provider-owned: Memorix and Engram use the LasmeX working directory's Git project, with Engram optionally accepting `ENGRAM_PROJECT`.

## Model guidance

The examples do not patch `lasmex-system-prompt`: a config patch replaces a row's complete config and could erase an existing persona. The README instead offers one optional additive instruction:
...
1. LasmeX session A calls a write tool and receives success for a unique value.
2. Fresh LasmeX session B, under the same provider storage scope, calls search or recall and returns that value without session A's transcript.
3. Session B uses the recalled value in a subsequent answer.

"Fresh session" means a new LasmeX session in the same Host. No Host restart is required. The generic MCP client discovers asynchronously and has no automatic reconnect after a child or HTTP transport closes; validation waits for tools before the first turn and uses HMR or a Host restart only after a crash.

## Alternatives considered

**One LasmeX plugin per provider.** Rejected because it repeats auth, configuration, lifecycle, and tool wrappers that MCP already standardizes and expands ownership for every added provider.

**A memory-provider preset registry.** Rejected because a registry would make third-party versions and recommendations look like a supported LasmeX product surface. Copyable overlays keep ownership and drift visible.

**Run `npx` or `go run` inside the MCP row.** Rejected after probes showed first-run npm downloads can exceed the MCP initialization timeout and an interrupted `npx` cache can become unusable. LasmeX starts a server process; it is not the provider package manager. Pinned installation commands are explicit prerequisites.
...
The earlier vendor-specific change is superseded by this generic path. Future provider drift is handled by updating and revalidating a small example pin rather than adding runtime branches to LasmeX.
