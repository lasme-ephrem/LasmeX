# Agent Note: The LasmeX CLI and personal config overlays from the Harness home
...
Two coupled pieces, aligned with the `apps/` assembly tier proposed by the `lasmex web` PR (#443):

**The `lasmex` CLI (`apps/cli`, npm name `lasmex`).** `apps/*` is the product-assembly tier over `packages/*` libraries. One bin dispatches the default interactive TUI, `-p`/`--prompt` headless turns, and the `web` surface. The TUI boots `examples/tui-agent/cordis.yml` (or `--config`) with the invoking directory as the workspace. From a source checkout, the root `pnpm lasmex` script runs the same entry with tsx's ESM hook without building; the [source-launch decision](../architecture/2026-07-29-dsh-source-launch-tsx-esm.md) owns the runtime vector and the [source-launch/build separation decision](../simplification/2026-08-12-separate-source-launch-from-build.md) owns artifact generation.

**Personal config (`lasmex-app-boot`).** The personal overlay lives in the Harness home — `$LASMEX_HOME`, else `~/.lasmex` — resolved by the shared [`resolveLasmexHome`](../architecture/2026-07-24-single-harness-home-resolver.md) (`lasmex-home-paths`), the same single root skills and AGENTS.md resolve against. The dsh TUI, Web, and headless surfaces consume its two optional files; the demo bins boot their committed trees verbatim:
...
The PTY smoke's launcher isolates `$LASMEX_HOME` to a per-test directory, exactly as it already isolates `LASMEX_AGENTS_HOME`, so a developer's real personal overlay cannot leak into fixtures; only the LasmeX CLI reads personal config, so no other test launcher needed changes.
...
- `lasmex-app-boot` depends on `js-yaml` and imports the include's `!!js` YAML dialect (`entryListSchema`) directly, and, like `apps/cli`, depends on `lasmex-home-paths` for `resolveLasmexHome`.
...
`packages/boot/app-boot/tests/user-patches.spec.ts` pins parsing, startup application, exact-path add/failure/recovery/removal, last-good rollback, failure broadcast, and preservation of app-owned patches. `apps/cli/tests/built-bin.e2e.ts` boots the real dsh bin over a profile and exercises the live patch layer end to end. Test launchers isolate `$LASMEX_HOME`, so a developer's real overlay cannot leak into fixtures.
