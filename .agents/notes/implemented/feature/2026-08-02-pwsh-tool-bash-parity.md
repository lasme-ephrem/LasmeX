...
- `lasmex-shell-env` is a new shipped package; `lasmex-tool-bash`'s `lasmexHome` config moved there, so compositions mounting the shell tools must also mount `shell-env` (the spine bundles do).
