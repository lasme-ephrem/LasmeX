...
进程内 subagent 继承父级的提供方、模型和输出上限。显式的 `SubagentStartRequest.agentOptions.maxTokens`（包括通过 `lasmex-tool-subagent` 配置的值）会覆盖该子级及其后代的继承值。进程外提供方自行持有其独立运行时的配置；因此 `subagent-lasmex-sdk` 公开独立的可选 `maxTokens`，并通过该子运行时自己的 SDK 握手传入。
