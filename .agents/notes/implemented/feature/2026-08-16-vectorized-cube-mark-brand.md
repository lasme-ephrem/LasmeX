...
Surfaces: the app favicon and the documentation-site favicon ship `lasmex-icon.svg` (`/favicon.svg`, SVG of size `any`), replacing the black LX monogram. The documentation-site navigation lockup inlines `lasmex-full.svg` at 44px instead of the black LX wordmark (`website/public/wordmark.svg` deleted, and the `LasmexMark`/`BrandWordmark` LX components removed with the monogram). The app's hero and narrow rail mount the traced cube mark (`CubeMark`). The desktop app ships the traced icon as `apps/desktop/assets/icon.svg`. The app's wide sidebar renders the full logo (`FullLogo`). `README.md`, `README.zh.md`, and `LASMEX.md` open with the full logo.
