# Agent Note: lasmex-hooks-claude-code + lasmex-hooks-codex — the Claude Code / Codex hook bridges
...
Matcher semantics, exit-code handling, and merge precedence live in `lasmex-hook-protocol`; each bridge only parses config, builds dialect payloads, and maps outcomes. Per-file coverage includes config branches plus end-to-end mappings through a real loop, `lasmex-bash-local`, and shell scripts, while a real-Loader smoke guards the package export shape. Native plugins bypass the wire protocol and return typed decisions directly.
