...
- `lasmex-session-persistence` is removed as a package.
- `lasmex-session` exports the persistence service type, coordinator, and contract helpers.
- JSONL and SQLite backend packages depend on `lasmex-session` directly.
