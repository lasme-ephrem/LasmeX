...
Three packages hand-roll promise-wrapped timers that the `node:timers/promises` builtin already provides, while other packages (`lasmex-llm-mock-server` `pause()`, `lasmex-lsp-stdio`, `lasmex-acp-snapshot`) already use the builtin — so the hand-rolled copies are also a consistency gap:
