...
Compaction is split between `lasmex-compaction`, which owns an abstract two-method service and shared types, and `lasmex-compaction-basic`, which owns the only complete provider. Shipped configurations load only the basic package, and no production package independently consumes the Service Definition package except that provider.
...
Move the basic implementation into `lasmex-compaction` and remove `lasmex-compaction-basic`. Keep `ctx.compaction`, `CompactionResult`, the shared transcript and tool-pairing helpers, the existing configuration, and the concrete compaction algorithm in one package.
...
- `lasmex-compaction-basic` and its workspace/package metadata are removed.
- `lasmex-compaction` owns the current configuration, plugin class, algorithm, types, events, and shared helpers.
